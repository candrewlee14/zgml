...
const abiStructModelLoadDesc = 3;
...
function expectTinyLlamaModelResourceBytes(device, resources, source, label) {
  ...
  for (const resource of resources) {
    ...
    const actual = device.readBytesSync(resource.descriptor, resource.byteLength);
    expectByteArray(actual, expected, `${label} ${resource.role}`);
  }
}

const wasiTinyLlamaFileName = "tiny-load.safetensors";
const wasiTempDir = await mkdtemp(join(tmpdir(), "zgml-wasi-"));
const wasiTinyLlamaGuestPath = wasiTinyLlamaFileName;
await writeFile(join(wasiTempDir, wasiTinyLlamaFileName), tinyLlamaSafetensorsFileBytes());
...
function abiStructSize(kind, label) {
  const size = Number(exports.zgml_abi_struct_size(kind));
...
function initAbiStructSizes() {
  ...
  modelLoadDescSize = abiStructSize(abiStructModelLoadDesc, "model load desc");
  ...
  if (exports.zgml_abi_struct_size(0) !== 0) throw new Error("unknown ABI struct kind returned a size");
}
...
function checkRuntimeInfo() {
  ...
  check(exports.zgml_get_runtime_info(info));
  ...
  if (
    ...
    throw new Error("unexpected zgml wasm runtime info");
  }
}
...
const hostDevice = new WasmWebGpuDevice({ registry: hostResources });
const keep = (ptr, size) => {
...
function createBuffer(byteLen) {
  ...
  check(exports.zgml_buffer_create(desc, out));
...
function wrapBuffer(dataPtr, byteLen) {
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_buffer_wrap(dataPtr, byteLen, out));
  const handle = u32(out);
...
function createProgramBuffer(program, kind) {
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_program_create_buffer(program, kind, out));
  const handle = u32(out);
...
function expectPortableDeviceBufferApisUnsupported(program, label) {
  ...
  expectStatus(exports.zgml_program_create_device_buffer(program, programBufferWeights, backendCpu, outBuffer), unsupported, `${label} CPU device-buffer factory`);
  if (u32(outBuffer) !== 0) throw new Error(`${label} CPU device-buffer factory returned a handle`);

  writeU32(outBuffer, 0, 0xffffffff);
  expectStatus(exports.zgml_program_create_device_buffer(program, programBufferWeights, backendWebGpu, outBuffer), unsupported, `${label} WebGPU device-buffer factory`);
  ...
  expectStatus(exports.zgml_program_get_device_handle(program, backendWebGpu, outDevice), unsupported, `${label} WebGPU device handle`);
  ...
  writeU32(outBuffer, 0, 0xffffffff);
  expectStatus(exports.zgml_program_import_device_buffer(program, programBufferWeights, importDesc, outBuffer), unsupported, `${label} WebGPU device-buffer import`);
  if (u32(outBuffer) !== 0) throw new Error(`${label} WebGPU device-buffer import returned a handle`);
}

function bufferSize(buffer) {
  return exports.zgml_buffer_size(buffer);
}

function inspectBuffer(buffer) {
  const out = keep(alloc(bufferInspectionSize), bufferInspectionSize);
  zero(out, bufferInspectionSize);
  check(exports.zgml_buffer_inspect(buffer, out));
  return out;
}

function bufferWrite(buffer, byteOffset, srcPtr, byteLen) {
  check(exports.zgml_buffer_write(buffer, byteOffset, srcPtr, byteLen));
}

function bufferRead(buffer, byteOffset, dstPtr, byteLen) {
  check(exports.zgml_buffer_read(buffer, byteOffset, dstPtr, byteLen));
}
...
function createModel(kind, inputLen, outputLen) {
  ...
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_model_create(desc, out));
  const handle = u32(out);
...
function inspectModel(model) {
  ...
  check(exports.zgml_model_inspect(model, out));
  return out;
}

function modelPathDesc(kind, modelPath) {
  const bytes = new TextEncoder().encode(modelPath);
  const pathPtr = keep(alloc(bytes.length), bytes.length);
  writeBytes(pathPtr, bytes);
  const desc = keep(alloc(modelLoadDescSize), modelLoadDescSize);
  zero(desc, modelLoadDescSize);
  writeU32(desc, 0, kind);
  writeU32(desc, 4, pathPtr);
  writeU32(desc, 8, bytes.length);
  return desc;
}

function probeModelPath(kind, modelPath) {
  const desc = modelPathDesc(kind, modelPath);
  const out = keep(alloc(modelInspectionSize), modelInspectionSize);
  zero(out, modelInspectionSize);
  const status = exports.zgml_model_probe_path(desc, out);
  return { status, out };
}

function loadModelPath(kind, modelPath) {
  const desc = modelPathDesc(kind, modelPath);
  const out = keep(alloc(handleSize), handleSize);
  writeU32(out, 0, 0xffffffff);
  const status = exports.zgml_model_load_path(desc, out);
...
function loadSafetensorsData(kind, bytes, reserved = 0) {
  ...
  const out = keep(alloc(handleSize), handleSize);
  writeU32(out, 0, 0xffffffff);
  const status = exports.zgml_model_load_safetensors_data(desc, out);
  return { status, handle: u32(out) };
}
...
function probeSafetensorsData(kind, bytes, reserved = 0) {
  ...
  const out = keep(alloc(modelInspectionSize), modelInspectionSize);
  zero(out, modelInspectionSize);
  const status = exports.zgml_model_probe_safetensors_data(desc, out);
  return { status, out };
}
...
function expectTinyLlamaLoadedModelStep(model, label) {
  ...
  try {
    ...
    if (session !== 0) exports.zgml_session_free(session);
    if (program !== 0) exports.zgml_program_free(program);
  }
}
...
function expectTinyLlama2LayerNativeKvBinding(model, label) {
  ...
  try {
    ...
    check(exports.zgml_llama_program_get_kv_cache_requirements(program, kvRequirements));
    if (
      ...
      throw new Error(`unexpected ${label} two-layer tiny LLaMA KV requirements: kind=${u32(kvRequirements, 0)} layers=${u32(kvRequirements, 8)} context=${u32(kvRequirements, 16)} kBytes=${u32(kvRequirements, 20)} vBytes=${u32(kvRequirements, 24)}`);
    }
    ...
    expectStatus(exports.zgml_llama_session_bind_buffers(program, badBindDesc, badOut), shapeMismatch, `${label} one-layer KV bind against two-layer checkpoint`);
...
function checkSupportedCheckpointCatalog() {
  const count = Number(exports.zgml_supported_checkpoint_count());
  if (count !== 4) throw new Error(`expected 4 supported checkpoint envelopes, got ${count}`);

  const out = keep(alloc(modelInspectionSize), modelInspectionSize);
  zero(out, modelInspectionSize);
  check(exports.zgml_supported_checkpoint_inspect(0, out));
  if (
  ...
  zero(out, modelInspectionSize);
  check(exports.zgml_supported_checkpoint_inspect(1, out));
  expectTinyLlamaCheckpointInspection(out, "two-layer supported catalog", { kind: tinyLlama2LayerKind, layers: 2 });

  zero(out, modelInspectionSize);
  check(exports.zgml_supported_checkpoint_inspect(2, out));
  expectSmolLMCheckpointInspection(out, "supported catalog");

  zero(out, modelInspectionSize);
  check(exports.zgml_supported_checkpoint_inspect(3, out));
  expectSmolLM2CheckpointInspection(out, "supported catalog");

  writeU32(out, 0, 99);
  writeU32(out, 24, 123);
  expectStatus(exports.zgml_supported_checkpoint_inspect(count, out), invalidArgument, "invalid supported checkpoint index");
...
function probeSafetensorsHeader(header, kind = autoKind, reserved = 0) {
  ...
  const out = keep(alloc(modelInspectionSize), modelInspectionSize);
  zero(out, modelInspectionSize);
  const status = exports.zgml_model_probe_safetensors_header(desc, out);
  return { status, out };
}
...
function programModelCompatibility(program, model) {
  ...
  check(exports.zgml_program_check_model_compatibility(program, model, out));
  return out;
}
...
function compileProgram(model, contextLength, backend = backendCpu) {
  ...
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_program_compile(model, desc, out));
  const handle = u32(out);
  if (handle === 0) throw new Error("program handle was not returned");
  return handle;
}

function runTinyLinearWebGpuCompileOnlySmoke() {
  let model = 0;
  let program = 0;
  try {
    ...
    const out = keep(alloc(handleSize), handleSize);
    writeU32(out, 0, 0xffffffff);
    expectStatus(exports.zgml_session_bind(program, bindDesc, out), unsupported, "WebGPU compile-only tiny linear bind");
    ...
    try {
      ...
      expectStatus(exports.zgml_session_step(resourceSession, 0, resourceStepResult), unsupported, "WebGPU tiny linear resource step");
      ...
      expectStatus(exports.zgml_session_step_no_output(resourceSession, 0, resourceStepResult), unsupported, "WebGPU tiny linear resource no-output step");
      ...
      exports.zgml_session_free(resourceSession);
    }
    ...
    const hostSessionHandle = hostSession.handle;
    try {
      const hostSessionInspection = inspectSession(hostSessionHandle);
      if (
        ...
        throw new Error("unexpected WebGPU tiny linear host Program resource-slot session inspection");
      }
      ...
      expectStatus(exports.zgml_session_step(hostSessionHandle, 0, hostStepResult), ok, "mock host WebGPU tiny linear native-export step");
      if (u32(hostStepResult) !== 3) throw new Error("mock host WebGPU tiny linear step did not write output_len");
      expectHostTinyLinearProfile(hostSession.runtimeProfile(), {
        ...
        syncCount: 0,
        lastOutputLength: 3,
      }, "mock host WebGPU tiny linear after logits step");
      expectClose(Array.from(hostDevice.readFloat32Sync(hostSession.bindings.output.descriptor, 3)), [2, 3, 5]);
      ...
      writeU32(hostStepResult, 0, 0xffffffff);
      expectStatus(exports.zgml_session_step_no_output(hostSessionHandle, 0, hostStepResult), ok, "mock host WebGPU tiny linear native-export no-output step");
      if (u32(hostStepResult) !== 0) throw new Error("mock host WebGPU tiny linear no-output step did not clear output_len");
      expectHostTinyLinearProfile(hostSession.runtimeProfile(), {
        callCount: 2,
        ...
        syncCount: 0,
        lastOutputLength: 0,
      }, "mock host WebGPU tiny linear after no-output step");
      expectClose(Array.from(hostDevice.readFloat32Sync(hostSession.bindings.output.descriptor, 3)), [4, 1, 5]);
      ...
      expectHostTinyLinearProfile(hostSession.runtimeProfile(), {
        ...
        syncCount: 0,
        lastOutputLength: 0,
      }, "mock host WebGPU tiny linear after profile reset");

      const wrappedHostExports = hostRuntime.wrapExports(exports);
      ...
      writeU32(hostStepResult, 0, 0xffffffff);
      const wrappedStepStatus = wrappedHostExports.zgml_session_step(hostSessionHandle, 0, hostStepResult);
      if (typeof wrappedStepStatus !== "number") throw new Error("wrapped host WebGPU tiny linear step did not return a synchronous status");
      expectStatus(wrappedStepStatus, ok, "wrapped mock host WebGPU tiny linear step");
      if (u32(hostStepResult) !== 3) throw new Error("wrapped mock host WebGPU tiny linear step did not write output_len");
      expectClose(Array.from(hostDevice.readFloat32Sync(hostSession.bindings.output.descriptor, 3)), [6, 2, 8]);
      expectHostTinyLinearProfile(hostSession.runtimeProfile(), {
        callCount: 1,
        backendDispatchCount: 1,
        noOutputCallCount: 0,
        ...
        syncCount: 0,
        lastOutputLength: 3,
      }, "wrapped mock host WebGPU tiny linear after logits step");
      ...
      writeU32(hostStepResult, 0, 0xffffffff);
      const wrappedNoOutputStatus = wrappedHostExports.zgml_session_step_no_output(hostSessionHandle, 0, hostStepResult);
      if (typeof wrappedNoOutputStatus !== "number") throw new Error("wrapped host WebGPU tiny linear no-output step did not return a synchronous status");
      expectStatus(wrappedNoOutputStatus, ok, "wrapped mock host WebGPU tiny linear no-output step");
      if (u32(hostStepResult) !== 0) throw new Error("wrapped mock host WebGPU tiny linear no-output step did not clear output_len");
      expectClose(Array.from(hostDevice.readFloat32Sync(hostSession.bindings.output.descriptor, 3)), [1, 7, 8]);
      expectHostTinyLinearProfile(hostSession.runtimeProfile(), {
        callCount: 2,
        backendDispatchCount: 2,
        noOutputCallCount: 1,
        ...
        syncCount: 0,
        lastOutputLength: 0,
      }, "wrapped mock host WebGPU tiny linear after no-output step");
    } finally {
      exports.zgml_session_free(hostSessionHandle);
      if (!hostSession.freed || hostRuntime.lookup(hostSessionHandle) !== null) {
        throw new Error("WebGPU tiny linear host Program Session was not released through native free");
      }
      if (hostRuntime.hasClaimed(hostSessionHandle)) {
        throw new Error("WebGPU tiny linear host Program Session claim was not released through native free");
      }
    }

    console.log("zgml wasm ffi webgpu compile/resource-probe tiny linear + mock host execution smoke ok");
  } finally {
    if (program !== 0) exports.zgml_program_free(program);
    if (model !== 0) exports.zgml_model_free(model);
  }
}
...
async function runTinyLlamaWebGpuCompileOnlySmoke() {
  ...
  try {
    ...
    check(exports.zgml_llama_program_inspect(program, llamaInspection));
    ...
    const out = keep(alloc(handleSize), handleSize);
    writeU32(out, 0, 0xffffffff);
    expectStatus(exports.zgml_session_bind(program, 0, out), unsupported, "WebGPU compile-only bind");
    ...
    expectStatus(exports.zgml_llama_session_bind_buffers(program, badOutputBindDesc, badOutputOut), unsupported, "WebGPU tiny LLaMA read-only output resource bind");
    ...
    expectStatus(exports.zgml_llama_session_bind_buffers(program, writeOnlyKvBindDesc, writeOnlyKvOut), unsupported, "WebGPU tiny LLaMA write-only KV resource bind");
    ...
    expectStatus(exports.zgml_llama_session_bind_buffers(program, readOnlyKvBindDesc, readOnlyKvOut), unsupported, "WebGPU tiny LLaMA read-only KV resource bind");
    ...
    let writeOnlyLlamaModelResourceError = "";
    try {
      ...
      writeOnlyLlamaModelResourceError = String(err && err.message ? err.message : err);
      writeOnlyLlamaModelResourceRejected = writeOnlyLlamaModelResourceError.includes("readable");
    }
    if (!writeOnlyLlamaModelResourceRejected) throw new Error(`expected WebGPU tiny LLaMA write-only model resource to reject: ${writeOnlyLlamaModelResourceError || "accepted"}`);
    ...
    try {
      ...
      const wrappedExports = hostRuntime.wrapExports(exports);
    ...
    try {
      ...
      const wrappedModelExports = hostRuntime.wrapExports(exports);
      ...
      exports.zgml_session_free(modelResourceSession);
    ...
    try {
      ...
      if (!hostDevice.canCreateGpuBuffers()) {
        expectClose(Array.from(hostDevice.readFloat32Sync(hookSession.bindings.output, tinyLlamaVocabSize)), [43, 44, 45, 46, 47, 48, 49, 50]);
      }
      ...
      if (!hostDevice.canCreateGpuBuffers()) {
        ...
        for (let i = 0; i < hookSession.stepParamsHistory.length; i += 1) {
          expectHostLlamaKvWindow(hookSession, hookSession.stepParamsHistory[i], `WebGPU tiny LLaMA host-token executor KV write ${i}`);
        }
      }
      ...
      expectStatus(exports.zgml_session_reset(hookSessionHandle), ok, "WebGPU tiny LLaMA host-token executor raw ABI reset");
      ...
      exports.zgml_session_free(hookSessionHandle);
    ...
    try {
      ...
      expectTinyLlamaModelResourceBytes(
        hostDevice,
        modelHookSession.modelResources,
      ...
      if (!hostDevice.canCreateGpuBuffers()) {
        expectHostLlamaKvWindow(modelHookSession, modelHookStepParams, "model-bound WebGPU tiny LLaMA host-token executor KV write");
      }
      ...
      if (u64(resetModelHookAbiProfile, 0) !== 0n || u64(resetModelHookAbiProfile, 24) !== 0n || u64(resetModelHookAbiProfile, 40) !== 0n) {
        ...
        exports.zgml_session_free(modelHookSessionHandle);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(projectionSession.bindings.output, tinyLlamaVocabSize)),
      ...
      exports.zgml_session_free(projectionSessionHandle);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(rmsNormProjectionSession.bindings.output, tinyLlamaVocabSize)),
      ...
      exports.zgml_session_free(rmsNormProjectionSessionHandle);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(kvProjectionK, expectedKvProjection.k.length, 0)),
        expectedKvProjection.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(kvProjectionV, expectedKvProjection.v.length, 0)),
        expectedKvProjection.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(kvProjectionK, expectedKvProjection.k.length, expectedKvProjection.k.length * Float32Array.BYTES_PER_ELEMENT)),
        Array(expectedKvProjection.k.length).fill(-707),
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(kvProjectionV, expectedKvProjection.v.length, expectedKvProjection.v.length * Float32Array.BYTES_PER_ELEMENT)),
        Array(expectedKvProjection.v.length).fill(-808),
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(kvProjectionSession.bindings.output, tinyLlamaVocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(kvProjectionK, expectedKvProjectionWindow.k.length, expectedKvProjection.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedKvProjectionWindow.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(kvProjectionV, expectedKvProjectionWindow.v.length, expectedKvProjection.v.length * Float32Array.BYTES_PER_ELEMENT)),
      ...
      exports.zgml_session_free(kvProjectionSessionHandle);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qkNormKvProjectionK, expectedQkNormKvProjection.k.length, 0)),
        expectedQkNormKvProjection.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qkNormKvProjectionV, expectedQkNormKvProjection.v.length, 0)),
      ...
      exports.zgml_session_free(qkNormKvProjectionSessionHandle);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qkNormAttentionProjectionK, expectedQkNormAttentionSeed.k.length, 0)),
        expectedQkNormAttentionSeed.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qkNormAttentionProjectionV, expectedQkNormAttentionSeed.v.length, 0)),
        expectedQkNormAttentionSeed.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qkNormAttentionProjectionSession.bindings.output, tinyLlamaVocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qkNormAttentionProjectionK, expectedQkNormAttentionDecode.k.length, expectedQkNormAttentionDecode.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedQkNormAttentionDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qkNormAttentionProjectionV, expectedQkNormAttentionDecode.v.length, expectedQkNormAttentionDecode.v.length * Float32Array.BYTES_PER_ELEMENT)),
      ...
      exports.zgml_session_free(qkNormAttentionProjectionSessionHandle);
    ...
    const slidingAttentionHostExports = hostRuntime.wrapExports(exports);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(slidingAttentionK, expectedSlidingAttentionFirst.k.length, 0)),
        expectedSlidingAttentionFirst.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(slidingAttentionV, expectedSlidingAttentionFirst.v.length, 0)),
        expectedSlidingAttentionFirst.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(slidingAttentionK, expectedSlidingAttentionSecond.k.length, expectedSlidingAttentionSecond.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedSlidingAttentionSecond.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(slidingAttentionV, expectedSlidingAttentionSecond.v.length, expectedSlidingAttentionSecond.v.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedSlidingAttentionSecond.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(slidingAttentionSession.bindings.output, groupedTinyLlamaShape.vocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(slidingAttentionK, expectedSlidingAttentionDecode.k.length, expectedSlidingAttentionDecode.k.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
        expectedSlidingAttentionDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(slidingAttentionV, expectedSlidingAttentionDecode.v.length, expectedSlidingAttentionDecode.v.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
    ...
    const llama3AttentionHostExports = hostRuntime.wrapExports(exports);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(llama3AttentionK, expectedLlama3AttentionPrefill.k.length, 0)),
        expectedLlama3AttentionPrefill.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(llama3AttentionV, expectedLlama3AttentionPrefill.v.length, 0)),
        expectedLlama3AttentionPrefill.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(llama3AttentionSession.bindings.output, groupedTinyLlamaShape.vocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(llama3AttentionK, expectedLlama3AttentionDecode.k.length, expectedLlama3AttentionDecode.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedLlama3AttentionDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(llama3AttentionV, expectedLlama3AttentionDecode.v.length, expectedLlama3AttentionDecode.v.length * Float32Array.BYTES_PER_ELEMENT)),
    ...
    const nopeAttentionHostExports = hostRuntime.wrapExports(exports);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(nopeAttentionK, expectedNopeAttentionPrefill.k.length, 0)),
        expectedNopeAttentionPrefill.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(nopeAttentionV, expectedNopeAttentionPrefill.v.length, 0)),
        expectedNopeAttentionPrefill.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(nopeAttentionSession.bindings.output, groupedTinyLlamaShape.vocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(nopeAttentionK, expectedNopeAttentionDecode.k.length, expectedNopeAttentionDecode.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedNopeAttentionDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(nopeAttentionV, expectedNopeAttentionDecode.v.length, expectedNopeAttentionDecode.v.length * Float32Array.BYTES_PER_ELEMENT)),
    ...
    const biasedQwen2AttentionHostExports = hostRuntime.wrapExports(exports);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2AttentionK, expectedBiasedQwen2AttentionPrefill.k.length, 0)),
        expectedBiasedQwen2AttentionPrefill.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2AttentionV, expectedBiasedQwen2AttentionPrefill.v.length, 0)),
        expectedBiasedQwen2AttentionPrefill.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2AttentionSession.bindings.output, groupedTinyLlamaShape.vocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2AttentionK, expectedBiasedQwen2AttentionDecode.k.length, expectedBiasedQwen2AttentionDecode.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedBiasedQwen2AttentionDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2AttentionV, expectedBiasedQwen2AttentionDecode.v.length, expectedBiasedQwen2AttentionDecode.v.length * Float32Array.BYTES_PER_ELEMENT)),
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionK, expectedAttentionSeed.k.length, 0)),
        expectedAttentionSeed.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionV, expectedAttentionSeed.v.length, 0)),
        expectedAttentionSeed.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionK, expectedAttentionSeed.k.length, expectedAttentionSeed.k.length * Float32Array.BYTES_PER_ELEMENT)),
        Array(expectedAttentionSeed.k.length).fill(-1111),
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionSession.bindings.output, tinyLlamaVocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionK, expectedAttentionDecode.k.length, expectedAttentionDecode.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedAttentionDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionV, expectedAttentionDecode.v.length, expectedAttentionDecode.v.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedAttentionDecode.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionK, expectedAttentionDecode.k.length, expectedAttentionDecode.k.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionK, expectedAttentionPrefillFirst.k.length, expectedAttentionPrefillFirst.k.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
        expectedAttentionPrefillFirst.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionV, expectedAttentionPrefillFirst.v.length, expectedAttentionPrefillFirst.v.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
        expectedAttentionPrefillFirst.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionK, expectedAttentionPrefillLast.k.length, expectedAttentionPrefillLast.k.length * 3 * Float32Array.BYTES_PER_ELEMENT)),
        expectedAttentionPrefillLast.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(attentionProjectionV, expectedAttentionPrefillLast.v.length, expectedAttentionPrefillLast.v.length * 3 * Float32Array.BYTES_PER_ELEMENT)),
      ...
      exports.zgml_session_free(attentionProjectionSessionHandle);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(inputBlockK, expectedInputBlock.k.length, 0)),
        expectedInputBlock.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(inputBlockV, expectedInputBlock.v.length, 0)),
        expectedInputBlock.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(inputBlockActivation, expectedInputBlock.blockHidden.length, 0)),
        expectedInputBlock.blockHidden,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionInputSession.bindings.output, tinyLlamaVocabSize)),
      ...
      exports.zgml_session_free(blockProjectionInputSessionHandle);
    }
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionK, expectedBlockSeed.k.length, 0)),
        expectedBlockSeed.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionV, expectedBlockSeed.v.length, 0)),
        expectedBlockSeed.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionActivationDescriptor, expectedBlockSeed.blockHidden.length, 0)),
        expectedBlockSeed.blockHidden,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionActivationDescriptor, expectedBlockSeed.blockHidden.length, expectedBlockSeed.blockHidden.length * Float32Array.BYTES_PER_ELEMENT)),
        Array(expectedBlockSeed.blockHidden.length).fill(-8888),
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionSession.bindings.output, tinyLlamaVocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionK, expectedBlockDecode.k.length, expectedBlockDecode.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedBlockDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionV, expectedBlockDecode.v.length, expectedBlockDecode.v.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedBlockDecode.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionActivationDescriptor, expectedBlockDecode.blockHidden.length, expectedBlockDecode.blockHidden.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedBlockDecode.blockHidden,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionK, expectedBlockDecode.k.length, expectedBlockDecode.k.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionK, expectedBlockPrefillFirst.k.length, expectedBlockPrefillFirst.k.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
        expectedBlockPrefillFirst.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionV, expectedBlockPrefillFirst.v.length, expectedBlockPrefillFirst.v.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
        expectedBlockPrefillFirst.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionK, expectedBlockPrefillLast.k.length, expectedBlockPrefillLast.k.length * 3 * Float32Array.BYTES_PER_ELEMENT)),
        expectedBlockPrefillLast.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionV, expectedBlockPrefillLast.v.length, expectedBlockPrefillLast.v.length * 3 * Float32Array.BYTES_PER_ELEMENT)),
        expectedBlockPrefillLast.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionActivationDescriptor, expectedBlockPrefillFirst.blockHidden.length, expectedBlockPrefillFirst.blockHidden.length * 2 * Float32Array.BYTES_PER_ELEMENT)),
        expectedBlockPrefillFirst.blockHidden,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockProjectionActivationDescriptor, expectedBlockPrefillLast.blockHidden.length, expectedBlockPrefillLast.blockHidden.length * 3 * Float32Array.BYTES_PER_ELEMENT)),
      ...
      exports.zgml_session_free(blockProjectionSessionHandle);
    ...
    const biasedQwen2BlockHostExports = hostRuntime.wrapExports(exports);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2BlockK, expectedBiasedQwen2BlockPrefill.k.length, 0)),
        expectedBiasedQwen2BlockPrefill.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2BlockV, expectedBiasedQwen2BlockPrefill.v.length, 0)),
        expectedBiasedQwen2BlockPrefill.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2BlockActivationDescriptor, expectedBiasedQwen2BlockPrefill.blockHidden.length, 0)),
        expectedBiasedQwen2BlockPrefill.blockHidden,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2BlockSession.bindings.output, groupedTinyLlamaShape.vocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2BlockK, expectedBiasedQwen2BlockDecode.k.length, expectedBiasedQwen2BlockDecode.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedBiasedQwen2BlockDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2BlockV, expectedBiasedQwen2BlockDecode.v.length, expectedBiasedQwen2BlockDecode.v.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedBiasedQwen2BlockDecode.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(biasedQwen2BlockActivationDescriptor, expectedBiasedQwen2BlockDecode.blockHidden.length, expectedBiasedQwen2BlockDecode.blockHidden.length * Float32Array.BYTES_PER_ELEMENT)),
    ...
    const qwen3QkNormBlockHostExports = hostRuntime.wrapExports(exports);
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qwen3QkNormBlockK, expectedQwen3QkNormBlockPrefill.k.length, 0)),
        expectedQwen3QkNormBlockPrefill.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qwen3QkNormBlockV, expectedQwen3QkNormBlockPrefill.v.length, 0)),
        expectedQwen3QkNormBlockPrefill.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qwen3QkNormBlockActivationDescriptor, expectedQwen3QkNormBlockPrefill.blockHidden.length, 0)),
        expectedQwen3QkNormBlockPrefill.blockHidden,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qwen3QkNormBlockSession.bindings.output, groupedTinyLlamaShape.vocabSize)),
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qwen3QkNormBlockK, expectedQwen3QkNormBlockDecode.k.length, expectedQwen3QkNormBlockDecode.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedQwen3QkNormBlockDecode.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qwen3QkNormBlockV, expectedQwen3QkNormBlockDecode.v.length, expectedQwen3QkNormBlockDecode.v.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedQwen3QkNormBlockDecode.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(qwen3QkNormBlockActivationDescriptor, expectedQwen3QkNormBlockDecode.blockHidden.length, expectedQwen3QkNormBlockDecode.blockHidden.length * Float32Array.BYTES_PER_ELEMENT)),
    ...
    const readStandaloneBlockFloat32 = async (descriptor, length, byteOffset = 0) => (
      Array.from(hostDevice.readFloat32Sync(descriptor, length, byteOffset))
    );
    async function runStandaloneBlockVariantProof(proof) {
      ...
      const hostExports = hostRuntime.wrapExports(exports);
      let modelPack = null;
      try {
    ...
    await runStandaloneBlockVariantProof({
      ...
      expectCall(call) {
        if (
          ...
          throw new Error(
            `host-only WebGPU SmolLM3 standalone block executor family evidence mismatch: ` +
            `layer=${call.layer} useRope=${call.useRope} biasMask=${call.biasMask} qkProjectionNorm=${call.qkProjectionNorm}`,
          );
        }
      },
    });
    ...
    const blockPipelineExecutor = new WasmWebGpuLlamaBlockPipelineExecutor({
      layers: [0, 0],
      record(call) {
        blockPipelineExecutorCalls.push(call);
      },
    ...
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(blockPipelineSession.bindings.output, tinyLlamaVocabSize)),
        expectedPipelineStage1Last.logits,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(pipelineK, expectedPipelineStage1First.k.length, 0)),
        expectedPipelineStage1First.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(pipelineV, expectedPipelineStage1First.v.length, 0)),
        expectedPipelineStage1First.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(pipelineK, expectedPipelineStage1Last.k.length, expectedPipelineStage1Last.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedPipelineStage1Last.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(pipelineV, expectedPipelineStage1Last.v.length, expectedPipelineStage1Last.v.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedPipelineStage1Last.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(pipelineActivation, expectedPipelineStage1First.blockHidden.length, 0)),
        expectedPipelineStage1First.blockHidden,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(pipelineActivation, expectedPipelineStage1Last.blockHidden.length, expectedPipelineStage1Last.blockHidden.length * Float32Array.BYTES_PER_ELEMENT)),
      ...
      exports.zgml_session_free(blockPipelineSessionHandle);
    ...
    const strictDefaultTwoLayerProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
      ...
      executorOptions: {
        record(call) {
          strictDefaultTwoLayerExecutorCalls.push(call);
        },
      },
    });
    ...
    const strictDefaultTwoLayerHostExports = hostRuntime.wrapExports(exports);
    try {
    ...
    const twoLayerPipelineProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
      ...
      executorOptions: {
        record(call) {
          twoLayerPipelineExecutorCalls.push(call);
        },
      },
    });
    ...
    const gpuRequiredTwoLayerHostExports = hostRuntime.wrapExports(exports);
    try {
      ...
      expectClose(Array.from(hostDevice.readFloat32Sync(gpuRequiredTwoLayerSession.bindings.output, tinyLlamaVocabSize)), gpuRequiredOutputSentinel);
      expectClose(Array.from(hostDevice.readFloat32Sync(gpuRequiredK, gpuRequiredKElements)), gpuRequiredKSentinel);
      expectClose(Array.from(hostDevice.readFloat32Sync(gpuRequiredV, gpuRequiredVElements)), gpuRequiredVSentinel);
    ...
    try {
      ...
      check(exports.zgml_llama_program_get_kv_cache_requirements(nativeDefaultTwoLayerProgramHandle, nativeDefaultTwoLayerKvRequirements));
      const nativeDefaultTwoLayerProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
        ...
        executorOptions: {
          record(call) {
            nativeDefaultTwoLayerExecutorCalls.push(call);
          },
        },
      });
      if (
      ...
      if (nativeDefaultTwoLayerSessionHandle !== 0) exports.zgml_session_free(nativeDefaultTwoLayerSessionHandle);
      ...
      if (nativeDefaultTwoLayerProgramHandle !== 0) exports.zgml_program_free(nativeDefaultTwoLayerProgramHandle);
      if (nativeDefaultTwoLayerCompatibleModel !== 0) exports.zgml_model_free(nativeDefaultTwoLayerCompatibleModel);
      if (nativeDefaultTwoLayerModel !== 0) exports.zgml_model_free(nativeDefaultTwoLayerModel);
    }
    ...
    const twoLayerHostExports = hostRuntime.wrapExports(exports);
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(twoLayerPipelineSession.bindings.output, tinyLlamaVocabSize)),
        expectedLayer1Last.logits,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(layer0K, expectedLayer0First.k.length, 0)),
        expectedLayer0First.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(layer0V, expectedLayer0First.v.length, 0)),
        expectedLayer0First.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(layer0K, expectedLayer0Last.k.length, expectedLayer0Last.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedLayer0Last.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(layer0V, expectedLayer0Last.v.length, expectedLayer0Last.v.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedLayer0Last.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(layer1K, expectedLayer1First.k.length, 0)),
        expectedLayer1First.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(layer1V, expectedLayer1First.v.length, 0)),
        expectedLayer1First.v,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(layer1K, expectedLayer1Last.k.length, expectedLayer1Last.k.length * Float32Array.BYTES_PER_ELEMENT)),
        expectedLayer1Last.k,
      );
      expectClose(
        Array.from(hostDevice.readFloat32Sync(layer1V, expectedLayer1Last.v.length, expectedLayer1Last.v.length * Float32Array.BYTES_PER_ELEMENT)),
      ...
      expectClose(Array.from(hostDevice.readFloat32Sync(layer0K, expectedResetLayer0.k.length, 0)), expectedResetLayer0.k);
      expectClose(Array.from(hostDevice.readFloat32Sync(layer0V, expectedResetLayer0.v.length, 0)), expectedResetLayer0.v);
      expectClose(Array.from(hostDevice.readFloat32Sync(layer1K, expectedResetLayer1.k.length, 0)), expectedResetLayer1.k);
      expectClose(Array.from(hostDevice.readFloat32Sync(layer1V, expectedResetLayer1.v.length, 0)), expectedResetLayer1.v);
      ...
      try {
        ...
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicSession.bindings.output, tinyLlamaVocabSize)),
        ...
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicLayer0K, expectedErgonomicLayer0First.k.length, 0)),
          expectedErgonomicLayer0First.k,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicLayer0V, expectedErgonomicLayer0First.v.length, 0)),
          expectedErgonomicLayer0First.v,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicLayer0K, expectedErgonomicLayer0Last.k.length, expectedErgonomicLayer0Last.k.length * Float32Array.BYTES_PER_ELEMENT)),
          expectedErgonomicLayer0Last.k,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicLayer0V, expectedErgonomicLayer0Last.v.length, expectedErgonomicLayer0Last.v.length * Float32Array.BYTES_PER_ELEMENT)),
          expectedErgonomicLayer0Last.v,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicLayer1K, expectedErgonomicLayer1First.k.length, 0)),
          expectedErgonomicLayer1First.k,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicLayer1V, expectedErgonomicLayer1First.v.length, 0)),
          expectedErgonomicLayer1First.v,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicLayer1K, expectedErgonomicLayer1Last.k.length, expectedErgonomicLayer1Last.k.length * Float32Array.BYTES_PER_ELEMENT)),
          expectedErgonomicLayer1Last.k,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(ergonomicLayer1V, expectedErgonomicLayer1Last.v.length, expectedErgonomicLayer1Last.v.length * Float32Array.BYTES_PER_ELEMENT)),
      ...
      try {
        ...
        expectClose(
          Array.from(hostDevice.readFloat32Sync(greedySession.bindings.output, tinyLlamaVocabSize)),
          expectedGreedyLayer1Second.logits,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(greedyLayer0K, expectedGreedyLayer0First.k.length, 0)),
          expectedGreedyLayer0First.k,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(greedyLayer0K, expectedGreedyLayer0Second.k.length, expectedGreedyLayer0Second.k.length * Float32Array.BYTES_PER_ELEMENT)),
          expectedGreedyLayer0Second.k,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(greedyLayer1V, expectedGreedyLayer1First.v.length, 0)),
          expectedGreedyLayer1First.v,
        );
        expectClose(
          Array.from(hostDevice.readFloat32Sync(greedyLayer1V, expectedGreedyLayer1Second.v.length, expectedGreedyLayer1Second.v.length * Float32Array.BYTES_PER_ELEMENT)),
      ...
      try {
        ...
        expectClose(
          Array.from(hostDevice.readFloat32Sync(deviceGreedySession.bindings.output, tinyLlamaVocabSize)),
      ...
      try {
        ...
        expectClose(Array.from(hostDevice.readFloat32Sync(greedyOverContextSession.bindings.output, tinyLlamaVocabSize)), greedyOverContextSentinel);
        expectClose(Array.from(hostDevice.readFloat32Sync(greedyOverContextK, greedyOverContextKElements)), greedyOverContextKSentinel);
        expectClose(Array.from(hostDevice.readFloat32Sync(greedyOverContextV, greedyOverContextVElements)), greedyOverContextVSentinel);
      ...
      try {
        ...
        expectClose(
          Array.from(hostDevice.readFloat32Sync(sampledSession.bindings.output, tinyLlamaVocabSize)),
        ...
        try {
          ...
          expectClose(
            Array.from(hostDevice.readFloat32Sync(deviceSampledSession.bindings.output, tinyLlamaVocabSize)),
            expectedSampledLayer1Second.logits,
          );
      ...
      try {
        ...
        expectClose(Array.from(hostDevice.readFloat32Sync(sampledOverContextSession.bindings.output, tinyLlamaVocabSize)), sampledOverContextSentinel);
        expectClose(Array.from(hostDevice.readFloat32Sync(sampledOverContextK, sampledOverContextKElements)), sampledOverContextKSentinel);
        expectClose(Array.from(hostDevice.readFloat32Sync(sampledOverContextV, sampledOverContextVElements)), sampledOverContextVSentinel);
    ...
    const shardedTwoLayerProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
      ...
      executorOptions: {
        record(call) {
          shardedTwoLayerExecutorCalls.push(call);
        },
      },
    });
	    if (
	    ...
	    try {
	      ...
	      check(exports.zgml_llama_program_get_kv_cache_requirements(nativeShardedProgramHandle, nativeShardedKvRequirements));
	      nativeShardedProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
	        ...
	        executorOptions: {
	          record(call) {
	            nativeShardedExecutorCalls.push(call);
	          },
	        },
	      });
	      if (
	      ...
	      if (nativeShardedSessionHandle !== 0) exports.zgml_session_free(nativeShardedSessionHandle);
	      ...
	      if (nativeShardedProgramHandle !== 0) exports.zgml_program_free(nativeShardedProgramHandle);
	      if (nativeShardedCompatibleModel !== 0) exports.zgml_model_free(nativeShardedCompatibleModel);
	      if (nativeShardedModel !== 0) exports.zgml_model_free(nativeShardedModel);
	    }
    ...
    const shardedTwoLayerHostExports = hostRuntime.wrapExports(exports);
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(shardedTwoLayerSession.bindings.output, tinyLlamaVocabSize)),
        expectedShardedLayer1.logits,
      );
      expectClose(Array.from(hostDevice.readFloat32Sync(shardedLayer0K, expectedShardedLayer0.k.length, 0)), expectedShardedLayer0.k);
      expectClose(Array.from(hostDevice.readFloat32Sync(shardedLayer0V, expectedShardedLayer0.v.length, 0)), expectedShardedLayer0.v);
      expectClose(Array.from(hostDevice.readFloat32Sync(shardedLayer1K, expectedShardedLayer1.k.length, 0)), expectedShardedLayer1.k);
      expectClose(Array.from(hostDevice.readFloat32Sync(shardedLayer1V, expectedShardedLayer1.v.length, 0)), expectedShardedLayer1.v);
    ...
    for (const dtypeProof of [
      ...
      const halfPipelineProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
        ...
        executorOptions: {
          record(call) {
            halfPipelineExecutorCalls.push(call);
          },
        },
      });
      ...
      try {
        ...
        check(exports.zgml_llama_program_get_kv_cache_requirements(nativeHalfProgramHandle, nativeHalfKvRequirements));
        nativeHalfProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
          ...
          executorOptions: {
            record(call) {
              nativeHalfExecutorCalls.push(call);
            },
          },
        });
        if (
        ...
        if (nativeHalfSessionHandle !== 0) exports.zgml_session_free(nativeHalfSessionHandle);
        ...
        if (nativeHalfProgramHandle !== 0) exports.zgml_program_free(nativeHalfProgramHandle);
        if (nativeHalfCompatibleModel !== 0) exports.zgml_model_free(nativeHalfCompatibleModel);
        if (nativeHalfModel !== 0) exports.zgml_model_free(nativeHalfModel);
      }
      ...
      const halfHostExports = hostRuntime.wrapExports(exports);
      try {
        ...
        expectClose(
          Array.from(hostDevice.readFloat32Sync(embeddingResource.descriptor, 4)),
        ...
        expectClose(
          Array.from(hostDevice.readFloat32Sync(halfPipelineSession.bindings.output, tinyLlamaVocabSize)),
          expectedLayer1.logits,
        );
        expectClose(Array.from(hostDevice.readFloat32Sync(layer0K, expectedLayer0.k.length, 0)), expectedLayer0.k);
        expectClose(Array.from(hostDevice.readFloat32Sync(layer0V, expectedLayer0.v.length, 0)), expectedLayer0.v);
        expectClose(Array.from(hostDevice.readFloat32Sync(layer1K, expectedLayer1.k.length, 0)), expectedLayer1.k);
        expectClose(Array.from(hostDevice.readFloat32Sync(layer1V, expectedLayer1.v.length, 0)), expectedLayer1.v);
    ...
    const threeLayerPipelineProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
      ...
      executorOptions: {
        record(call) {
          threeLayerPipelineExecutorCalls.push(call);
        },
      },
    });
    if (
    ...
    const threeLayerHostExports = hostRuntime.wrapExports(exports);
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(threeLayerPipelineSession.bindings.output, tinyLlamaVocabSize)),
      ...
      for (let layer = 0; layer < 3; layer += 1) {
        for (let tokenIndex = 0; tokenIndex < expectedThreeLayer.length; tokenIndex += 1) {
          ...
          expectClose(Array.from(hostDevice.readFloat32Sync(threeLayerCaches[layer].k, expected.k.length, offset)), expected.k);
          expectClose(Array.from(hostDevice.readFloat32Sync(threeLayerCaches[layer].v, expected.v.length, offset)), expected.v);
        }
      }
    ...
    const widerPipelineProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
      ...
      executorOptions: {
        record(call) {
          widerPipelineExecutorCalls.push(call);
        },
      },
    });
    if (
    ...
    const widerHostExports = hostRuntime.wrapExports(exports);
    try {
      if (
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(widerPipelineSession.bindings.output, widerTinyLlamaShape.vocabSize)),
        widerOutputSentinel,
      );
      ...
      for (let layer = 0; layer < widerTinyLlamaShape.layers; layer += 1) {
        for (let tokenIndex = 0; tokenIndex < expectedWider.length; tokenIndex += 1) {
          ...
          const offset = tokenIndex * expected.k.length * Float32Array.BYTES_PER_ELEMENT;
          expectClose(Array.from(hostDevice.readFloat32Sync(widerCaches[layer].k, expected.k.length, offset)), expected.k);
          expectClose(Array.from(hostDevice.readFloat32Sync(widerCaches[layer].v, expected.v.length, offset)), expected.v);
        }
      }
    ...
    const realisticHeadPipelineProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
      ...
      executorOptions: {
        record(call) {
          realisticHeadPipelineExecutorCalls.push(call);
        },
      },
    });
    if (
    ...
    const realisticHeadHostExports = hostRuntime.wrapExports(exports);
    try {
      if (
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(realisticHeadPipelineSession.bindings.output, realisticHeadTinyLlamaShape.vocabSize)),
      ...
      for (let tokenIndex = 0; tokenIndex < expectedRealisticHead.length; tokenIndex += 1) {
        ...
        const offset = tokenIndex * expected.k.length * Float32Array.BYTES_PER_ELEMENT;
        expectClose(Array.from(hostDevice.readFloat32Sync(realisticHeadCaches[0].k, expected.k.length, offset)), expected.k, 1e-3);
        expectClose(Array.from(hostDevice.readFloat32Sync(realisticHeadCaches[0].v, expected.v.length, offset)), expected.v, 1e-3);
      }
    ...
    const packedTinyLlamaProofs = [
      ...
      const recordPackedLlamaProfileLabel = (label) => {
        ...
        packedLlamaProfileLabels.push(label);
	        ...
	        if (nativeOutputMismatchProgramHandle !== 0) exports.zgml_program_free(nativeOutputMismatchProgramHandle);
		        if (nativeOutputMismatchModel !== 0) exports.zgml_model_free(nativeOutputMismatchModel);
		      }
		    }
		      ...
		      try {
		        ...
		        if (nativeModelCompatibilityMismatchProgramHandle !== 0) exports.zgml_program_free(nativeModelCompatibilityMismatchProgramHandle);
		        if (nativeModelCompatibilityMismatchBoundModel !== 0) exports.zgml_model_free(nativeModelCompatibilityMismatchBoundModel);
			        if (nativeModelCompatibilityMismatchModel !== 0) exports.zgml_model_free(nativeModelCompatibilityMismatchModel);
			      }
			    }

		    {
		      ...
		      try {
		        ...
		        if (nativeEnvelopeMismatchProgramHandle !== 0) exports.zgml_program_free(nativeEnvelopeMismatchProgramHandle);
		        if (nativeEnvelopeMismatchModel !== 0) exports.zgml_model_free(nativeEnvelopeMismatchModel);
		      }
		    }
      ...
      const groupedPipelineProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
        ...
        executorOptions: {
          record(call) {
            groupedPipelineExecutorCalls.push(call);
          },
        },
      });
      ...
      if (nativeBackedDefaultFamilyLabels.has(packedTinyLlamaProof.label)) {
        ...
        try {
          ...
          check(exports.zgml_llama_program_get_kv_cache_requirements(nativeFamilyProgramHandle, nativeFamilyKvRequirements));
          nativeFamilyProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
            ...
            executorOptions: {
              record(call) {
                nativeFamilyExecutorCalls.push(call);
              },
            },
          });
          if (
          ...
          nativeFamilyProgram?.tokenExecutor?.destroy?.();
	          if (nativeFamilyProgramHandle !== 0) exports.zgml_program_free(nativeFamilyProgramHandle);
	          if (nativeFamilyCompatibleModel !== 0) exports.zgml_model_free(nativeFamilyCompatibleModel);
	          if (nativeFamilyModel !== 0) exports.zgml_model_free(nativeFamilyModel);
	        }
	      }
	      if (nativeBackedMismatchedFamilyLabels.has(packedTinyLlamaProof.label)) {
	        ...
	        try {
	          ...
	          if (nativeMismatchProgramHandle !== 0) exports.zgml_program_free(nativeMismatchProgramHandle);
	          if (nativeMismatchCompatibleModel !== 0) exports.zgml_model_free(nativeMismatchCompatibleModel);
	          if (nativeMismatchModel !== 0) exports.zgml_model_free(nativeMismatchModel);
	        }
	      }
      ...
      const groupedHostExports = hostRuntime.wrapExports(exports);
      try {
      expectHostResourceTableRoles(
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(groupedPipelineSession.bindings.output, groupedTinyLlamaShape.vocabSize)),
      ...
      for (let layer = 0; layer < groupedTinyLlamaShape.layers; layer += 1) {
        for (let tokenIndex = 0; tokenIndex < expectedGrouped.length; tokenIndex += 1) {
          ...
          const offset = tokenIndex * expected.k.length * Float32Array.BYTES_PER_ELEMENT;
          expectClose(Array.from(hostDevice.readFloat32Sync(groupedCaches[layer].k, expected.k.length, offset)), expected.k, groupedTolerance);
          expectClose(Array.from(hostDevice.readFloat32Sync(groupedCaches[layer].v, expected.v.length, offset)), expected.v, groupedTolerance);
        }
      }
      ...
      try {
        ...
        expectClose(
          Array.from(hostDevice.readFloat32Sync(groupedGreedySession.bindings.output, groupedTinyLlamaShape.vocabSize)),
        ...
        for (let layer = 0; layer < groupedTinyLlamaShape.layers; layer += 1) {
          for (let tokenIndex = 0; tokenIndex < expectedGroupedGreedy.length; tokenIndex += 1) {
            ...
            const offset = tokenIndex * expected.k.length * Float32Array.BYTES_PER_ELEMENT;
            expectClose(Array.from(hostDevice.readFloat32Sync(groupedGreedyCaches[layer].k, expected.k.length, offset)), expected.k, groupedTolerance);
            expectClose(Array.from(hostDevice.readFloat32Sync(groupedGreedyCaches[layer].v, expected.v.length, offset)), expected.v, groupedTolerance);
          }
        }
    ...
    try {
      ...
      const customLmHeadBiasHostExports = hostRuntime.wrapExports(exports);
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(customLmHeadBiasSession.bindings.output, tinyLlamaVocabSize)),
    ...
    try {
      ...
      const customRootHostExports = hostRuntime.wrapExports(exports);
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(customRootSession.bindings.output, tinyLlamaVocabSize)),
    ...
    if (!untiedMissingLmHeadRejected) {
      ...
      executorOptions: {
        record(call) {
          tiedPipelineExecutorCalls.push(call);
	        },
	      },
	    });
	    ...
	    try {
	      ...
	      check(exports.zgml_llama_program_get_kv_cache_requirements(nativeTiedProgramHandle, nativeTiedKvRequirements));
	      nativeTiedProgram = WasmWebGpuLlamaResourceProgram.fromSafetensors({
	        ...
	        executorOptions: {
	          record(call) {
	            nativeTiedExecutorCalls.push(call);
	          },
	        },
	      });
	      if (
	      ...
	      if (nativeTiedSessionHandle !== 0) exports.zgml_session_free(nativeTiedSessionHandle);
	      ...
	      if (nativeTiedProgramHandle !== 0) exports.zgml_program_free(nativeTiedProgramHandle);
	      if (nativeTiedCompatibleModel !== 0) exports.zgml_model_free(nativeTiedCompatibleModel);
	      if (nativeTiedModel !== 0) exports.zgml_model_free(nativeTiedModel);
	    }
    ...
    const tiedHostExports = hostRuntime.wrapExports(exports);
    try {
      ...
      expectClose(
        Array.from(hostDevice.readFloat32Sync(tiedPipelineSession.bindings.output, tinyLlamaVocabSize)),
        tiedExpectedLayer1.logits,
      );
      expectClose(Array.from(hostDevice.readFloat32Sync(tiedLayer0K, tiedExpectedLayer0.k.length, 0)), tiedExpectedLayer0.k);
      expectClose(Array.from(hostDevice.readFloat32Sync(tiedLayer0V, tiedExpectedLayer0.v.length, 0)), tiedExpectedLayer0.v);
      expectClose(Array.from(hostDevice.readFloat32Sync(tiedLayer1K, tiedExpectedLayer1.k.length, 0)), tiedExpectedLayer1.k);
      expectClose(Array.from(hostDevice.readFloat32Sync(tiedLayer1V, tiedExpectedLayer1.v.length, 0)), tiedExpectedLayer1.v);
    ...
    const expectRejectedHostLlamaTokenExecutor = (executor, slug, label, options = {}) => {
      ...
      try {
        ...
        exports.zgml_session_free(invalidSessionHandle);
    ...
    try {
      ...
      exports.zgml_session_free(invalidTableHashSessionHandle);
    ...
    try {
      ...
      exports.zgml_session_free(invalidModelHandleSessionHandle);
    ...
    try {
      ...
      exports.zgml_session_free(missingModelEvidenceSessionHandle);
    ...
    for (const implicitModelEvidenceProof of [
      ...
      try {
        ...
        exports.zgml_session_free(implicitModelEvidenceSessionHandle);
    ...
    try {
      ...
      exports.zgml_session_free(invalidModelTableHashSessionHandle);
    ...
    try {
      ...
      exports.zgml_session_free(invalidModelManifestHashSessionHandle);
    ...
    try {
      ...
      exports.zgml_session_free(missingModelPackEvidenceSessionHandle);
    ...
    try {
      ...
      exports.zgml_session_free(invalidModelPackHashSessionHandle);
    ...
    console.log("zgml wasm ffi webgpu resource-probe tiny llama smoke ok");
  } finally {
    if (program !== 0) exports.zgml_program_free(program);
    if (compatibleModel !== 0) exports.zgml_model_free(compatibleModel);
    if (model !== 0) exports.zgml_model_free(model);
  }
}
...
function inspectProgram(program) {
  ...
  check(exports.zgml_program_inspect(program, inspection));
  return inspection;
}
...
function programRequirements(program) {
  ...
  check(exports.zgml_program_get_requirements(program, requirements));
  return requirements;
}
...
function programRuntimeProfile(program) {
  ...
  check(exports.zgml_program_runtime_profile(program, profile));
  return profile;
}

function sessionRuntimeProfile(session) {
  return sessionRuntimeProfileWith(exports, session);
}
...
function expectHostLlamaKvWindow(session, stepParams, label) {
  ...
  for (let layer = 0; layer < kvCache.length; layer += 1) {
    for (const kind of ["k", "v"]) {
      ...
      const actual = Array.from(session.device.readFloat32Sync(descriptor, length, offset));
...
function inspectSession(session) {
  return inspectSessionWith(exports, session);
}
...
function resetSessionRuntimeProfile(session) {
  check(exports.zgml_session_reset_runtime_profile(session));
}
...
function bindSession(program, desc = 0) {
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_session_bind(program, desc, out));
  const handle = u32(out);
...
function bindBufferSession(program, desc) {
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_session_bind_buffers(program, desc, out));
  const handle = u32(out);
...
function bindModelSession(program, model, desc = 0) {
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_session_bind_model(program, model, desc, out));
  const handle = u32(out);
...
function bindModelBufferSession(program, model, desc) {
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_session_bind_model_buffers(program, model, desc, out));
  const handle = u32(out);
...
function bindLlamaBufferSession(program, desc = 0) {
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_llama_session_bind_buffers(program, desc, out));
  const handle = u32(out);
...
function bindLlamaModelBufferSession(program, model, desc = 0) {
  const out = keep(alloc(handleSize), handleSize);
  check(exports.zgml_llama_session_bind_model_buffers(program, model, desc, out));
  const handle = u32(out);
...
function sessionPosition(session) {
  return sessionPositionWith(exports, session);
}
...
function sessionReset(session) {
  check(exports.zgml_session_reset(session));
}
...
function tokenStep(session, token, outputPtr, outputLen) {
  ...
  const status = exports.zgml_session_step_token(session, desc, result);
...
function tokenAdvance(session, token) {
  ...
  writeU32(desc, 0, token);
  return exports.zgml_session_advance_token(session, desc);
}
...
function tokenAdvanceTokens(session, tokensPtr, tokensLen) {
  ...
  return exports.zgml_session_advance_tokens(session, desc);
}
...
function tokenPrefill(session, tokensPtr, tokensLen, outputPtr, outputLen) {
  ...
  writeU32(desc, 12, outputLen);
  const result = keep(alloc(stepResultSize), stepResultSize);
  const status = exports.zgml_session_prefill_tokens(session, desc, result);
  return { status, outputLen: u32(result) };
}

function tokenExecute(session, tokensPtr, tokensLen, outputPolicy, outputPtr, outputLen) {
  return tokenExecuteWith(exports, session, tokensPtr, tokensLen, outputPolicy, outputPtr, outputLen);
}
...
function tokenArgmax(session, logitsPtr = 0, logitsLen = 0) {
  ...
  const status = exports.zgml_session_argmax_token(session, desc, result);
  return {
    status,
    token: u32(result),
...
function tokenExecuteArgmax(session, tokensPtr, tokensLen) {
  ...
  writeU32(desc, 8, 0);
  const result = keep(alloc(tokenArgmaxResultSize), tokenArgmaxResultSize);
  const status = exports.zgml_session_execute_argmax_tokens(session, desc, result);
  return {
    status,
    token: u32(result),
    logit: view().getFloat32(result + 4, true),
...
function tokenGenerateArgmax(session, tokensPtr, tokensLen, outputTokensPtr, outputTokensLen) {
  ...
  const status = exports.zgml_session_generate_argmax_tokens(session, desc, result);
  return {
    status,
...
function tokenSample(session, logitsPtr = 0, logitsLen = 0, topK = 1, seed = 123, temperature = 1.0) {
  ...
  const status = exports.zgml_session_sample_token(session, desc, result);
  return {
    status,
    token: u32(result),
    logit: view().getFloat32(result + 4, true),
...
function tokenExecuteSample(session, tokensPtr, tokensLen, topK = 1, seed = 123, temperature = 1.0) {
  ...
  writeU32(desc, 20, 0);
  const result = keep(alloc(tokenSampleResultSize), tokenSampleResultSize);
  const status = exports.zgml_session_execute_sample_tokens(session, desc, result);
  return {
    status,
    token: u32(result),
    logit: view().getFloat32(result + 4, true),
...
function tokenGenerateSample(session, tokensPtr, tokensLen, outputTokensPtr, outputTokensLen, topK = 1, seed = 123, temperature = 1.0) {
  ...
  const status = exports.zgml_session_generate_sample_tokens(session, desc, result);
  return {
    status,
    tokensGenerated: u32(result),
    lastToken: u32(result, 4),
...
function runTinyLinearSmoke() {
  ...
  try {
    ...
    const result = keep(alloc(stepResultSize), stepResultSize);
    check(exports.zgml_session_step(session, stepDesc, result));
    ...
    check(exports.zgml_session_step(boundSession, 0, result));
    ...
    check(exports.zgml_session_step(wrappedSession, 0, result));
    ...
    expectStatus(exports.zgml_buffer_write(resourceWeightsBuffer, 0, wrappedWeights, 4), unsupported, "resource buffer write");
    expectStatus(exports.zgml_buffer_read(resourceWeightsBuffer, 0, wrappedWeights, 4), unsupported, "resource buffer read");
    ...
    expectStatus(exports.zgml_session_bind_buffers(program, resourceBindDesc, resourceOut), unsupported, "host-only resource binding");
    if (u32(resourceOut) !== 0) throw new Error("resource bind returned a session handle");

    console.log(`zgml wasm ffi smoke ok: [${values.join(", ")}]`);
  } finally {
    if (wrappedSession !== 0) exports.zgml_session_free(wrappedSession);
    if (boundSession !== 0) exports.zgml_session_free(boundSession);
    if (session !== 0) exports.zgml_session_free(session);
    if (program !== 0) exports.zgml_program_free(program);
    if (model !== 0) exports.zgml_model_free(model);
  }
...
async function runTinyLlamaSmoke() {
  let model = 0;
  let compatibleModel = 0;
  let program = 0;
  const sessions = [];
  try {
    ...
    check(exports.zgml_llama_program_inspect(program, llamaInspection));
    if (
      u64(llamaInspection, 0) !== 8n ||
      u64(llamaInspection, 8) !== 8n ||
      u64(llamaInspection, 16) !== 4n ||
      u64(llamaInspection, 24) !== 1n ||
      u64(llamaInspection, 40) !== 1n ||
      u64(llamaInspection, 48) !== 1n ||
      u64(llamaInspection, 56) !== 1n ||
      u64(llamaInspection, 72) !== 1n ||
      u64(llamaInspection, 96) === 0n
    ) {
      throw new Error("unexpected tiny LLaMA inspection");
    }
    if (
      u64(llamaInspection, 96) !== u64(inspection, 72) ||
      u64(llamaInspection, 104) !== u64(inspection, 80) ||
      u64(llamaInspection, 112) !== u64(inspection, 88)
    ) {
    ...
    const resourceOut = keep(alloc(handleSize), handleSize);
    writeU32(resourceOut, 0, 0xffffffff);
    expectStatus(exports.zgml_session_bind_buffers(program, resourceBindDesc, resourceOut), unsupported, "host-only tiny LLaMA resource output binding");
    if (u32(resourceOut) !== 0) throw new Error("resource-output tiny LLaMA bind returned a session handle");
    writeU32(resourceOut, 0, 0xffffffff);
    expectStatus(exports.zgml_session_bind_model_buffers(program, compatibleModel, resourceBindDesc, resourceOut), unsupported, "host-only model-bound tiny LLaMA resource output binding");
    ...
    const hostKvOutputLen = hostKvStep.outputLen;
    const hostKvSentinel = readF32Array(hostKvLogits + tinyLlamaVocabSize * 4, 1)[0];
    if (hostKvOutputLen !== tinyLlamaVocabSize || hostKvSentinel !== -4545) {
      throw new Error(`unexpected host-KV tiny LLaMA step: outputLen=${hostKvOutputLen} sentinel=${hostKvSentinel}`);
    }
    ...
    writeU32(resourceOut, 0, 0xffffffff);
    expectStatus(exports.zgml_llama_session_bind_buffers(program, llamaResourceBindDesc, resourceOut), unsupported, "host-only tiny LLaMA KV resource binding");
    if (u32(resourceOut) !== 0) throw new Error("KV-resource tiny LLaMA bind returned a session handle");
    writeU32(resourceOut, 0, 0xffffffff);
    expectStatus(exports.zgml_llama_session_bind_model_buffers(program, compatibleModel, llamaResourceBindDesc, resourceOut), unsupported, "host-only model-bound tiny LLaMA KV resource binding");
    ...
    const stepB = tokenStep(b, 0, logitsB, tinyLlamaVocabSize + 1);
    check(stepB.status);
    ...
    const advancedStep = tokenStep(advanced, 1, advancedLogits, tinyLlamaVocabSize + 1);
    check(advancedStep.status);
    ...
    console.log(`zgml wasm ffi tiny llama smoke ok: ${tinyLlamaVocabSize} logits`);
  } finally {
    for (let i = sessions.length - 1; i >= 0; i -= 1) exports.zgml_session_free(sessions[i]);
    if (program !== 0) exports.zgml_program_free(program);
    if (compatibleModel !== 0) exports.zgml_model_free(compatibleModel);
    if (model !== 0) exports.zgml_model_free(model);
  }
...
function runWasiTinyLlamaCheckpointLoadSmoke() {
  const modelPath = wasiTinyLlamaGuestPath;
  let autoModel = 0;
  let fixedModel = 0;
  let program = 0;
  let session = 0;
  try {
    const autoProbe = probeModelPath(autoKind, modelPath);
    check(autoProbe.status);
    expectTinyLlamaCheckpointInspection(autoProbe.out, "auto WASI path probe");

    const fixedProbe = probeModelPath(tinyLlamaKind, modelPath);
    check(fixedProbe.status);
    expectTinyLlamaCheckpointInspection(fixedProbe.out, "fixed WASI path probe");
    expectStatus(probeModelPath(smollm135mKind, modelPath).status, unsupported, "SmolLM WASI tiny checkpoint probe");

    const wrongLoad = loadModelPath(smollm135mKind, modelPath);
    expectStatus(wrongLoad.status, unsupported, "SmolLM WASI tiny checkpoint load");
    if (wrongLoad.handle !== 0) throw new Error("SmolLM WASI tiny checkpoint load returned a model handle");

    const autoLoad = loadModelPath(autoKind, modelPath);
    check(autoLoad.status);
    autoModel = autoLoad.handle;
    if (autoModel === 0) throw new Error("auto WASI tiny checkpoint load returned no model handle");
    expectTinyLlamaCheckpointInspection(inspectModel(autoModel), "auto-loaded WASI tiny checkpoint");

    const fixedLoad = loadModelPath(tinyLlamaKind, modelPath);
    check(fixedLoad.status);
    fixedModel = fixedLoad.handle;
    if (fixedModel === 0) throw new Error("fixed WASI tiny checkpoint load returned no model handle");
    expectTinyLlamaCheckpointInspection(inspectModel(fixedModel), "fixed-loaded WASI tiny checkpoint");

    program = compileProgram(autoModel, 4);
    const compatibility = programModelCompatibility(program, fixedModel);
    if (
      u32(compatibility, 0) !== tinyLlamaKind ||
      u32(compatibility, 4) !== tinyLlamaKind ||
      u64(compatibility, 8) !== 1n
    ) {
      throw new Error("unexpected WASI checkpoint model compatibility");
    }

    expectTinyLlamaLoadedModelStep(autoModel, "WASI path-loaded");

    console.log(`zgml wasm ffi WASI tiny checkpoint load smoke ok: ${modelPath}`);
  } finally {
    if (session !== 0) exports.zgml_session_free(session);
    if (program !== 0) exports.zgml_program_free(program);
    if (fixedModel !== 0) exports.zgml_model_free(fixedModel);
    if (autoModel !== 0) exports.zgml_model_free(autoModel);
  }
}
...
function runTinyLlamaSafetensorsDataLoadSmoke() {
  ...
  try {
    ...
    console.log("zgml wasm ffi tiny checkpoint data load smoke ok");
  } finally {
    if (fixedTwoLayerModel !== 0) exports.zgml_model_free(fixedTwoLayerModel);
    if (autoTwoLayerModel !== 0) exports.zgml_model_free(autoTwoLayerModel);
    if (fixedModel !== 0) exports.zgml_model_free(fixedModel);
    if (autoModel !== 0) exports.zgml_model_free(autoModel);
  }
}
...
try {
  ...
  runWasiTinyLlamaCheckpointLoadSmoke();
  runTinyLinearSmoke();
  runTinyLinearWebGpuCompileOnlySmoke();
  await runTinyLlamaSmoke();
  await runTinyLlamaWebGpuCompileOnlySmoke();
} finally {
  for (let i = buffers.length - 1; i >= 0; i -= 1) {
    exports.zgml_buffer_free(buffers[i]);
  }
  for (let i = allocations.length - 1; i >= 0; i -= 1) {
    const [ptr, size] = allocations[i];
    free(ptr, size);
  }
  await rm(wasiTempDir, { recursive: true, force: true });
}
