...
function assertHotRuntimeProfile(target: Pick<Program, "requireHotRuntimeProfile" | "runtimeProfile" | "runtimeProfileHasNoFallback" | "runtimeProfileHasNoSync" | "runtimeProfileHasNoInvalidRuntimePatches">, label: string): void {
...
function assertProgramEvidence(program: Program, target: NnCompilableModule, requirements: ProgramRequirements): void {
  ...
  const compatibility = program.moduleCompatibility(target);
  const executionPlan = program.executionPlan(target);
...
const rejectedCompatibility = session.stepParamsCompatibility({ input: tensor([1, 2, 3], [3] as const), output: false });

assertClose(eager.data[0] ?? Number.NaN, -0.5, 1e-6, "eager output");
assertClose(compiled.data[0] ?? Number.NaN, eager.data[0] ?? Number.NaN, 1e-6, "compiled output");
...
console.log(`zgml bun Program/Session smoke ok: eager=${(eager.data[0] ?? Number.NaN).toFixed(6)} compiled=${(compiled.data[0] ?? Number.NaN).toFixed(6)}`);
