const zgml = require("../..");
const nodeAdapter = require("../../dist/node.cjs");
if (
  nodeAdapter.Tensor !== zgml.Tensor ||
  nodeAdapter.nn !== zgml.nn ||
  nodeAdapter.TinyLinear !== zgml.TinyLinear ||
  nodeAdapter.Program !== zgml.Program ||
  nodeAdapter.Session !== zgml.Session
) {
  throw new Error("root package entrypoint must re-export the Node FFI wrapper");
}
const { NativeBuffer, Program, Session, SmolLM135M, Tensor, TinyLinear, TinyLinearProgram, TinyLinearSession, TinyLlama, abiStructSize, abiStructSizes, arange, cat, checkpoint, full, linspace, loadedRuntimeInfo, loadModel, loadSafetensorsData, loss, nn, ones, optim, param, parameter, probeModel, probeSafetensorsData, probeSafetensorsHeader, rand, randn, runtimeInfo, scalar, stack, supportedCheckpointModels, tensor, train, webgpuInterop, zeros } = zgml;
...
function expectCloseWithin(actual, expected, tolerance) {
  if (actual.length !== expected.length) {
    throw new Error(`expected ${expected.length} outputs, got ${actual.length}`);
  }
  for (let i = 0; i < actual.length; i += 1) {
...
function expectCapabilityDispatchPlan(capabilities, inspection, label) {
  const expectedFull =
  ...
  if (
    ...
    throw new Error(`unexpected ${label} capability dispatch plan: ${JSON.stringify({ capabilities, inspection }, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
  }
}
...
function tinyLlamaSafetensorsHeader() {
  const tensors = {};
...
function writeTinySafetensorsFile(filePath, header) {
  const headerBytes = Buffer.from(header);
  const len = Buffer.alloc(8);
  len.writeBigUInt64LE(BigInt(headerBytes.length), 0);
  fs.writeFileSync(filePath, Buffer.concat([len, headerBytes]));
}

function safetensorsHeaderFileBytes(header) {
  const headerBytes = Buffer.from(header);
  const len = Buffer.alloc(8);
  len.writeBigUInt64LE(BigInt(headerBytes.length), 0);
  return Buffer.concat([len, headerBytes]);
}

function tinyLlamaSafetensorsFileBytes() {
  const tensors = {};
  let offset = 0;
  for (const [name, shape] of tinyLlamaTensorSpecs) {
  ...
  while ((8 + Buffer.byteLength(header)) % Float32Array.BYTES_PER_ELEMENT !== 0) {
    header += " ";
  }
  const headerBytes = Buffer.from(header);
  const len = Buffer.alloc(8);
  len.writeBigUInt64LE(BigInt(headerBytes.length), 0);
  return Buffer.concat([len, headerBytes, Buffer.alloc(offset)]);
}

function writeTinyLlamaSafetensorsFile(filePath) {
  fs.writeFileSync(filePath, tinyLlamaSafetensorsFileBytes());
}

function expectTinyLlamaCheckpointInspection(model, label) {
  ...
  if (
    ...
    throw new Error(`unexpected ${label} model inspection: ${JSON.stringify(inspection)}`);
  }
}

function expectTinyLlamaCheckpointStep(model, label) {
  let program = null;
  let session = null;
  try {
    ...
    for (const value of logits) {
      if (Math.abs(value) > 1e-6) {
        throw new Error(`expected zero ${label} checkpoint logits, got ${Array.from(logits)}`);
      }
    }
  } finally {
...
function isShapeMismatchError(err) {
  return (
    err &&
    (err.status === "shape_mismatch" || err.code === 3 || String(err.message || err).includes("shape_mismatch"))
  );
}

function expectRuntimeProfileStable(before, after, label) {
  const fields = [
    "callCount",
    "backendOpCount",
    "fallbackOpCount",
  ...
  for (const field of fields) {
    if (before[field] !== after[field]) {
      throw new Error(`unexpected ${label} profile delta for ${field}: ${JSON.stringify({ before, after }, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
  }
}
...
function isUnsupportedError(err) {
  return (
    err &&
    (err.status === "unsupported" || err.code === 5 || String(err.message || err).includes("unsupported"))
  );
}

function isImportRejectedError(err) {
  return (
    err &&
    (err.code === 3 ||
      err.code === 5 ||
      err.status === "shape_mismatch" ||
      err.status === "unsupported" ||
      String(err.message || err).includes("shape_mismatch") ||
      String(err.message || err).includes("unsupported"))
  );
}
...
if (
  ...
  throw new Error(`unexpected zgml runtime info: ${JSON.stringify(info, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
}
...
if (
  ...
  throw new Error(`unexpected zgml ABI struct sizes: ${JSON.stringify(abiSizes)}`);
}
let unknownAbiStructRejected = false;
try {
  abiStructSize("missingStruct");
} catch {
...
if (typeof SmolLM135M.load !== "function") {
  throw new Error("expected fixed SmolLM135M Node facade");
}
if (typeof loadModel !== "function") {
  throw new Error("expected root loadModel Node facade");
}
if (typeof probeModel !== "function" || typeof SmolLM135M.probe !== "function") {
  throw new Error("expected model path probe Node facade");
}
if (typeof probeSafetensorsHeader !== "function" || typeof SmolLM135M.probeSafetensorsHeader !== "function") {
  throw new Error("expected safetensors header probe Node facade");
}
...
if (
  ...
  throw new Error(`unexpected supported checkpoint catalog: ${JSON.stringify(supportedCheckpoints)}`);
}
const headerProbe = probeSafetensorsHeader(smollmSafetensorsHeader());
const fixedHeaderProbe = SmolLM135M.probeSafetensorsHeader(Buffer.from(smollmSafetensorsHeader()));
const tinyHeaderProbe = probeSafetensorsHeader(tinyLlamaSafetensorsHeader());
const fixedTinyHeaderProbe = TinyLlama.probeSafetensorsHeader(Buffer.from(tinyLlamaSafetensorsHeader()));
...
const tempSafetensorsDir = fs.mkdtempSync(path.join(os.tmpdir(), "zgml-safetensors-probe-"));
...
try {
  const tempSafetensorsPath = path.join(tempSafetensorsDir, "model.safetensors");
  writeTinySafetensorsFile(tempSafetensorsPath, smollmSafetensorsHeader());
  fileHeaderProbe = probeModel(tempSafetensorsPath);
  const tinySafetensorsPath = path.join(tempSafetensorsDir, "tiny-model.safetensors");
  ...
  const tinyLoadPath = path.join(tempSafetensorsDir, "tiny-load.safetensors");
  ...
  try {
    ...
    smollmTinyProbeRejected = String(err && err.message || err).includes("unsupported");
  }
  ...
  try {
    ...
    smollmTinyRejected = String(err && err.message || err).includes("unsupported");
  }
  ...
  try {
    ...
    smollmTinyBytesRejected = String(err && err.message || err).includes("unsupported");
  }
  if (!smollmTinyBytesRejected) {
    throw new Error("expected SmolLM135M.loadSafetensorsData to reject tiny LLaMA safetensors");
  }
} finally {
  fs.rmSync(tempSafetensorsDir, { recursive: true, force: true });
}
if (
  ...
  throw new Error(`unexpected safetensors header probe: ${JSON.stringify({ headerProbe, fixedHeaderProbe, fileHeaderProbe, tinyHeaderProbe, fixedTinyHeaderProbe, tinyFileHeaderProbe })}`);
}
let unknownLoadModelKindRejected = false;
try {
  loadModel("does-not-need-to-exist.gguf", { kind: "missing-family" });
} catch (err) {
  unknownLoadModelKindRejected = String(err && err.message || err).includes("unknown zgml model load kind");
}
...
try {
  probeModel("does-not-need-to-exist.gguf", { kind: "missing-family" });
} catch (err) {
  unknownProbeModelKindRejected = String(err && err.message || err).includes("unknown zgml model load kind");
}
...
try {
  ...
  if (
    ...
    throw new Error(`unexpected tiny linear model inspection: ${JSON.stringify(modelInspection)}`);
  }
    ...
    if (
      ...
      throw new Error(`unexpected tiny linear capabilities: ${JSON.stringify(capabilities, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected tiny linear requirements: ${JSON.stringify(requirements)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected tiny linear buffer layout: ${JSON.stringify(bufferLayout)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected tiny linear model compatibility: ${JSON.stringify(compatibility)}`);
    }
    ...
    try {
      ...
      badBindRejected = String(err && err.message || err).includes("Program weights slot length 6");
    }
    ...
    try {
      ...
      if (!Object.isFrozen(programProfile) || programProfile.callCount !== 0 || programProfile.runtimePatchCallCount !== 0) {
        throw new Error(`expected cold program profile, got ${JSON.stringify(programProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
      ...
      if (
        ...
        throw new Error(`unexpected session profile: ${JSON.stringify(sessionProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
      ...
      if (!Object.isFrozen(resetProfile) || resetProfile.callCount !== 0 || resetProfile.runtimePatchCallCount !== 0 || resetProfile.commandCount === 0) {
        throw new Error(`unexpected reset session profile: ${JSON.stringify(resetProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
      console.log(`zgml node ffi smoke ok: [${Array.from(output).join(", ")}]`);
    } finally {
      session.free();
    }
    ...
    if (!(frontendInput instanceof Tensor) || frontendInput.rank !== 1 || frontendInput.length !== 2) {
      throw new Error("unexpected JS Tensor frontend input shape");
    }
    ...
    if (
      ...
      throw new Error("expected JS Tensor shape evidence to be frozen");
    }
    if (frontendInput.dtype !== "f32" || frontendInput.device !== "cpu" || frontendInput.to("cpu") !== frontendInput || frontendInput.to("float32") !== frontendInput || frontendInput.cpu() !== frontendInput || frontendInput.float() !== frontendInput || frontendInput.float32() !== frontendInput) {
      throw new Error("unexpected JS Tensor dtype/device movement surface");
    }
    ...
    if (movedCopy === frontendInput || movedCopy.dtype !== "f32" || movedCopy.device !== "cpu") {
      throw new Error("expected JS Tensor.to copy to return a distinct f32 cpu tensor");
    }
    ...
    try {
      ...
      implicitDeviceRejected = String(err && err.message || err).includes("Tensor.place(program, kind)");
    }
    if (!implicitDeviceRejected) throw new Error("expected JS Tensor.to webgpu to reject without a compiled Program");
    ...
    if (!(dropout instanceof nn.Dropout) || dropout.kind !== "dropout" || dropout.p !== 0.5 || !dropout.training) {
      throw new Error("unexpected JS nn.Dropout/dropout surface");
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS dropout compile support: ${JSON.stringify(dropoutSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS zero-dropout compile support: ${JSON.stringify(zeroDropoutSupport)}`);
    }
    ...
    if (runtimeReportsNativeWgpu) {
      ...
      try {
        ...
        if (
          ...
          throw new Error(`unexpected JS zero-dropout WebGPU inspection: ${JSON.stringify({ zeroDropoutWebgpuInspection, zeroDropoutWebgpuCapabilities }, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
        }
        ...
        try {
          ...
          if (
            ...
            throw new Error(`unexpected JS zero-dropout WebGPU host profile: ${JSON.stringify(profile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
          }
        } finally {
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              throw new Error(`unexpected JS zero-dropout WebGPU device-buffer session inspection: ${JSON.stringify(deviceSessionInfo, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
            }
            ...
            if (
              ...
              throw new Error(`unexpected JS zero-dropout WebGPU device-buffer profile: ${JSON.stringify(deviceProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
            }
          } finally {
    ...
    if (dropout.eval() !== dropout || dropout.training || dropout.forward(dropoutEvalInput) !== dropoutEvalInput) {
      throw new Error("expected JS dropout eval mode to be an identity Tensor path");
    }
    dropout.train();
    const dropoutSeq = nn.sequential([dropout]);
    if (!dropoutSeq.training || !dropout.training) throw new Error("expected JS dropout Sequential to start in train mode");
    dropoutSeq.eval();
    if (dropoutSeq.training || dropout.training) throw new Error("expected JS Sequential.eval to cascade to dropout");
    dropoutSeq.train();
    if (!dropoutSeq.training || !dropout.training) throw new Error("expected JS Sequential.train to cascade to dropout");
    ...
    if (
      ...
      throw new Error(`unexpected JS eval dropout compile support: ${JSON.stringify(evalDropoutSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS eval dropout compile evidence: ${JSON.stringify(evalDropoutEvidence)}`);
      }
    ...
    if (
      ...
      throw new Error(`unexpected JS eval dropout Sequential compile support: ${JSON.stringify(evalDropoutSeqSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS elided dropout-linear compile support: ${JSON.stringify(elidedDropoutLinearSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS elided dropout-linear compile evidence: ${JSON.stringify({ elidedInspection, elidedEvidence })}`);
      }
      ...
      if (!elidedCompatibility.compatible || elidedDropoutLinearProgram.acceptsModule(elidedDropoutLinear, { inputShape: [4] }) !== true) {
        throw new Error(`unexpected JS elided dropout-linear compatibility: ${JSON.stringify(elidedCompatibility)}`);
      }
      ...
      if (
        ...
        throw new Error(`expected JS elided IR compatibility mismatch: ${JSON.stringify(identityIdentityCompatibility)}`);
      }
      let identityIdentityBindRejected = false;
      try {
        elidedDropoutLinearProgram.bindModule(identityIdentityLinear, { inputShape: [4] });
      } catch (err) {
        identityIdentityBindRejected = String(err && err.message || err).includes("Tensor Program IR");
      }
      if (!identityIdentityBindRejected) throw new Error("expected JS bindModule to reject mismatched elided Tensor Program IR");
    ...
    if (explicitBroadcastedBias.shape.join(",") !== "2,3") throw new Error(`unexpected JS broadcastTo shape: ${explicitBroadcastedBias.shape.join(",")}`);
    ...
    if (explicitExpanded.shape.join(",") !== "2,3") throw new Error(`unexpected JS expand shape: ${explicitExpanded.shape.join(",")}`);
    ...
    try {
      ...
      explicitBroadcastRejected = String(err && err.message || err).includes("cannot broadcast");
    }
    if (!explicitBroadcastRejected) throw new Error("expected JS broadcastTo incompatible shape to reject");
    const broadcastModuleInput = tensor([10, 20, 30], [3], { requiresGrad: true });
    const broadcastModule = nn.broadcastTo([2, 3]);
    if (!(broadcastModule instanceof nn.Shape)) throw new Error("expected JS nn.broadcastTo to create a Shape module");
    const broadcastModuleOut = broadcastModule.forward(broadcastModuleInput);
    if (broadcastModuleOut.shape.join(",") !== "2,3") throw new Error(`unexpected JS nn.broadcastTo shape: ${broadcastModuleOut.shape.join(",")}`);
    ...
    if (flattenedByModule.shape.join(",") !== "4") throw new Error(`unexpected JS nn.flatten shape: ${flattenedByModule.shape.join(",")}`);
    const flattenCtorModule = new nn.Flatten();
    if (!(flattenCtorModule instanceof nn.Shape) || flattenCtorModule.kind !== "flatten") throw new Error("expected JS nn.Flatten to create a Shape flatten module");
    const flattenedByCtor = flattenCtorModule.forward(tensor([1, 2, 3, 4], [2, 2]));
    if (flattenedByCtor.shape.join(",") !== "4") throw new Error(`unexpected JS nn.Flatten shape: ${flattenedByCtor.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error("expected JS nn.identity/Identity to create Shape identity modules");
    }
    const identityInput = tensor([1, 2, 3, 4], [2, 2]);
    if (identityModule.forward(identityInput) !== identityInput) throw new Error("expected JS nn.identity to return the input Tensor");
    ...
    if (
      ...
      throw new Error(`unexpected JS nn.identity compile support: ${JSON.stringify(identitySupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS nn.identity compile evidence: ${JSON.stringify(identityEvidence)}`);
      }
    ...
    if (!(viewModule instanceof nn.Shape) || viewModule.kind !== "view") throw new Error("expected JS nn.view to create a Shape module");
    const viewedByModule = viewModule.forward(tensor([1, 2, 3, 4], [4]));
    if (viewedByModule.shape.join(",") !== "2,2") throw new Error(`unexpected JS nn.view shape: ${viewedByModule.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error(`unexpected JS nn.view compile support: ${JSON.stringify(viewSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS nn.view compile evidence: ${JSON.stringify(viewEvidence)}`);
      }
    ...
    if (!(unsqueezeModule instanceof nn.Shape) || unsqueezeModule.kind !== "unsqueeze") throw new Error("expected JS nn.unsqueeze to create a Shape module");
    const unsqueezedByModule = unsqueezeModule.forward(tensor([1, 2, 3], [3]));
    if (unsqueezedByModule.shape.join(",") !== "1,3") throw new Error(`unexpected JS nn.unsqueeze shape: ${unsqueezedByModule.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error(`unexpected JS nn.unsqueeze compile support: ${JSON.stringify(unsqueezeSupport)}`);
    }
    ...
    if (!(squeezeModule instanceof nn.Shape) || squeezeModule.kind !== "squeeze" || squeezeModule.squeezeAll) throw new Error("expected JS nn.squeeze(dim) to create a Shape module");
    const squeezedByModule = squeezeModule.forward(tensor([1, 2, 3], [1, 3]));
    if (squeezedByModule.shape.join(",") !== "3") throw new Error(`unexpected JS nn.squeeze shape: ${squeezedByModule.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error(`unexpected JS nn.squeeze compile support: ${JSON.stringify(squeezeSupport)}`);
    }
    ...
    if (!squeezeAllModule.squeezeAll || squeezeAllModule.forward(tensor([7], [1, 1])).shape.join(",") !== "1") {
      throw new Error("unexpected JS nn.squeeze() all-dim semantics");
    }
    const transposeModule = nn.transpose();
    if (!(transposeModule instanceof nn.Shape) || transposeModule.kind !== "transpose") throw new Error("expected JS nn.transpose to create a Shape module");
    const transposedByModule = transposeModule.forward(tensor([1, 2, 3, 4, 5, 6], [2, 3]));
    if (transposedByModule.shape.join(",") !== "3,2") throw new Error(`unexpected JS nn.transpose shape: ${transposedByModule.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error(`unexpected JS nn.transpose compile support: ${JSON.stringify(transposeSupport)}`);
    }
    ...
    if (
      ...
      throw new Error("unexpected JS shape-module trace");
    }
    if (
    ...
    if (
      ...
      throw new Error(`unexpected JS shape-module compile support: ${JSON.stringify(shapeSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS shape-chain compile support: ${JSON.stringify(shapeChainSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS shape-chain compile evidence: ${JSON.stringify(evidence)}`);
      }
    ...
    if (!(rankNarrow instanceof nn.Shape) || rankNarrow.kind !== "narrow") throw new Error("expected JS nn.narrow to create a Shape module");
    const rankNarrowOut = rankNarrow.forward(tensor([10, 20, 30, 40], [4]));
    if (rankNarrowOut.shape.join(",") !== "2") throw new Error(`unexpected JS nn.narrow rank-1 shape: ${rankNarrowOut.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error(`unexpected JS rank-1 narrow compile support: ${JSON.stringify(rankNarrowSupport)}`);
    }
    ...
    try {
      ...
      if (!rankNarrowEvidence || rankNarrowEvidence.kind !== "module" || rankNarrowEvidence.kernelPlan.ops[0].kernel !== "narrow") {
        throw new Error(`unexpected JS rank-1 narrow evidence: ${JSON.stringify(rankNarrowEvidence)}`);
      }
    ...
    if (
      ...
      throw new Error(`unexpected JS batch-axis narrow compile support: ${JSON.stringify(batchNarrowSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS single-batch feature narrow compile support: ${JSON.stringify(singleBatchFeatureSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS strided feature narrow compile support: ${JSON.stringify(stridedFeatureSupport)}`);
    }
    ...
    if (!(scalarSelect instanceof nn.Shape) || scalarSelect.kind !== "select") throw new Error("expected JS nn.select to create a Shape module");
    const scalarSelectOut = scalarSelect.forward(tensor([10, 20, 30], [3]));
    if (scalarSelectOut.shape.join(",") !== "1") throw new Error(`unexpected JS rank-1 select shape: ${scalarSelectOut.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error(`unexpected JS rank-1 select compile support: ${JSON.stringify(scalarSelectSupport)}`);
    }
    ...
    if (rowSelectOut.shape.join(",") !== "3") throw new Error(`unexpected JS row select shape: ${rowSelectOut.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error(`unexpected JS row select compile support: ${JSON.stringify(rowSelectSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS single-batch feature select compile support: ${JSON.stringify(singleBatchFeatureSelectSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS strided feature select compile support: ${JSON.stringify(stridedFeatureSelectSupport)}`);
    }
    ...
    if (!(rankSlice instanceof nn.Shape) || rankSlice.kind !== "slice") throw new Error("expected JS nn.slice to create a Shape module");
    const rankSliceOut = rankSlice.forward(tensor([10, 20, 30, 40], [4]));
    if (rankSliceOut.shape.join(",") !== "2") throw new Error(`unexpected JS rank-1 slice shape: ${rankSliceOut.shape.join(",")}`);
    ...
    if (
      ...
      throw new Error(`unexpected JS rank-1 slice compile support: ${JSON.stringify(rankSliceSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS row slice compile support: ${JSON.stringify(rowSliceSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS single-batch feature slice compile support: ${JSON.stringify(singleBatchFeatureSliceSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS batched feature slice compile support: ${JSON.stringify(batchedFeatureSliceSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS stepped slice compile support: ${JSON.stringify(steppedSliceSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS stepped feature slice compile support: ${JSON.stringify(steppedFeatureSliceSupport)}`);
    }
    ...
    if (catRows.shape.join(",") !== "3,2") throw new Error(`unexpected JS cat shape: ${catRows.shape.join(",")}`);
    ...
    if (catCols.shape.join(",") !== "2,4") throw new Error(`unexpected JS cat dim -1 shape: ${catCols.shape.join(",")}`);
    ...
    if (stackedRows.shape.join(",") !== "2,2") throw new Error(`unexpected JS stack shape: ${stackedRows.shape.join(",")}`);
    ...
    if (stackedLast.shape.join(",") !== "2,2") throw new Error(`unexpected JS stack dim -1 shape: ${stackedLast.shape.join(",")}`);
    ...
    if (zeros([2, 2]).shape.join(",") !== "2,2" || !zeros([2, 2], { requiresGrad: true }).requiresGrad) {
      throw new Error("unexpected JS zeros factory metadata");
    }
    ...
    if (scalar(4, { requiresGrad: true }).shape.join(",") !== "1" || !scalar(4, { requiresGrad: true }).requiresGrad) {
      throw new Error("unexpected JS scalar factory metadata");
    }
    ...
    if (deterministicRandn.shape.join(",") !== "2" || !Array.from(deterministicRandn.toFloat32Array()).every(Number.isFinite)) {
      throw new Error("unexpected JS randn factory output");
    }
    ...
    if (!standaloneParamAlias.requiresGrad) {
      throw new Error("unexpected JS param alias metadata");
    }
    ...
    if (indexedTensor.get([1, 2]) !== 6 || indexedTensor.get(1, -1) !== 6) {
      throw new Error("unexpected JS Tensor get result");
    }
    ...
    if (JSON.stringify(indexedTensor.toArray()) !== JSON.stringify([[1, 20, 3], [40, 5, 6]]) || JSON.stringify(indexedTensor.tolist()) !== JSON.stringify(indexedTensor.toArray())) {
      throw new Error("unexpected JS Tensor nested array conversion");
    }
    const selectedTensor = indexedTensor.select(0, -1);
    if (selectedTensor.shape.join(",") !== "3") throw new Error(`unexpected JS Tensor select shape: ${selectedTensor.shape.join(",")}`);
    expectClose(selectedTensor.toFloat32Array(), [40, 5, 6]);
    const narrowedTensor = indexedTensor.narrow(1, 1, 2);
    if (narrowedTensor.shape.join(",") !== "2,2") throw new Error(`unexpected JS Tensor narrow shape: ${narrowedTensor.shape.join(",")}`);
    expectClose(narrowedTensor.toFloat32Array(), [20, 3, 5, 6]);
    const slicedTensor = indexedTensor.slice(1, 0, null, 2);
    if (slicedTensor.shape.join(",") !== "2,2") throw new Error(`unexpected JS Tensor slice shape: ${slicedTensor.shape.join(",")}`);
    ...
    if (frontendInput.sum().item() !== 3 || frontendInput.mean().item() !== 1.5) {
      throw new Error("unexpected JS Tensor reduction result");
    }
    const dimReduce = tensor([1, 2, 3, 4, 5, 6], [2, 3], { requiresGrad: true });
    const dimSum0 = dimReduce.sumDim(0);
    if (dimSum0.shape.join(",") !== "1,3") {
      throw new Error(`unexpected JS sumDim(0) shape: ${dimSum0.shape.join(",")}`);
    }
    ...
    if (dimReduce.max().item() !== 6) {
      throw new Error("unexpected JS Tensor max reduction result");
    }
    ...
    if (restoredTensor.shape.join(",") !== "2") {
      throw new Error("unexpected JS Tensor JSON shape");
    }
    ...
    if (clonedTensor.toFloat32Array() === frontendInput.toFloat32Array()) {
      throw new Error("unexpected JS Tensor clone alias");
    }
    ...
    if (detachedTensor.requiresGrad || detachedTensor.toFloat32Array() === cloneSource.toFloat32Array()) {
      throw new Error("unexpected JS Tensor detach semantics");
    }
    if (frontendInput.reshape([1, 2]).shape.join(",") !== "1,2" || frontendInput.view([-1, 1]).shape.join(",") !== "2,1") {
      throw new Error("unexpected JS Tensor reshape shape");
    }
    if (
      ...
      throw new Error("unexpected JS Tensor squeeze/unsqueeze shape");
    }
    ...
    if (tensor([1, 2, 3, 4], [2, 2]).flatten().shape.join(",") !== "4") {
      throw new Error("unexpected JS Tensor flatten shape");
    }
    ...
    if (transposeOutput.shape.join(",") !== "3,2") {
      throw new Error("unexpected JS Tensor transpose shape");
    }
    ...
    if (loss.meanSquaredError(new Tensor(frontendHostOutput, [3]), [9.5, 11.5, 16.0]).item() !== 0 || loss.mse(frontendHostOutput, frontendHostOutput).item() !== 0) {
      throw new Error("unexpected JS nn/loss frontend host result");
    }
    ...
    if (!(batchedOutput instanceof Tensor) || batchedOutput.shape.join(",") !== "2,3") {
      throw new Error("expected JS batched linear output to be a [batch, outFeatures] Tensor");
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS nn parameter list: ${JSON.stringify(frontendParams.map((param) => param.name))}`);
    }
    ...
    if (frontendNamed.length !== 2 || frontendNamed[0].name !== "0.weight" || nn.namedParameters(frontend)[1].name !== "0.bias") {
      throw new Error(`unexpected JS nn named parameter list: ${JSON.stringify(frontendNamed.map((param) => param.name))}`);
    }
    ...
    if (frontendState["0.weight"].shape.join(",") !== "2,3") {
      throw new Error(`unexpected JS stateDict weight shape: ${frontendState["0.weight"].shape.join(",")}`);
    }
    if (frontendState["0.weight"].layout !== "row-major:linear.weight[in_features,out_features]") {
      throw new Error(`unexpected JS stateDict weight layout: ${frontendState["0.weight"].layout}`);
    }
    if (frontendState["0.bias"].layout !== "row-major:linear.bias[out_features]") {
      throw new Error(`unexpected JS stateDict bias layout: ${frontendState["0.bias"].layout}`);
    }
    ...
    try {
      ...
      shapeStateRejected = String(err && err.message || err).includes("shape must be [2,3]");
    }
    if (!shapeStateRejected) throw new Error("expected JS stateDict shape rejection");
    ...
    try {
      ...
      layoutStateRejected = String(err && err.message || err).includes("layout must be row-major:linear.weight[in_features,out_features]");
    }
    if (!layoutStateRejected) throw new Error("expected JS stateDict layout rejection");
    ...
    try {
      ...
      strictStateRejected = String(err && err.message || err).includes("unexpected parameter extra");
    }
    if (!strictStateRejected) throw new Error("expected JS stateDict strict extra key rejection");
    frontend.loadStateDict({
      "0.weight": [1, 2, 3, 4, 5, 6],
      "0.bias": [0.5, -0.5, 1.0],
    });
    ...
    const checkpointSnapshot = checkpoint.create({
      ...
      metadata: { epoch: 1, tag: "node-smoke" },
    });
    ...
    if (
      ...
      checkpointRoundTrip.metadata.tag !== "node-smoke" ||
      ...
      throw new Error(`unexpected JS checkpoint snapshot: ${JSON.stringify(checkpointRoundTrip)}`);
    }
    ...
    if (!Array.isArray(checkpoint.moduleStateDict(restoredCheckpointModel)["0.weight"].data)) {
      throw new Error("expected JS checkpoint module state to be JSON-safe");
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS single-linear compile support: ${JSON.stringify(frontendSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS root single-linear compile evidence: ${JSON.stringify(rootFrontendEvidence)}`);
      }
      ...
      try {
        ...
        if (!(rootFrontendTensor instanceof Tensor) || rootFrontendTensor.shape.join(",") !== "3") {
          throw new Error(`unexpected JS root single-linear stepTensor shape: ${rootFrontendTensor.shape}`);
        }
    ...
    try {
      ...
      if (!frontendEvidence || frontendEvidence.kind !== "tiny-linear" || frontendEvidence.trace.ops[0].op !== "linear") {
        throw new Error(`unexpected JS single-linear compile evidence: ${JSON.stringify(frontendEvidence)}`);
      }
    ...
    if (
      ...
      throw new Error(`unexpected JS mismatched linear compile support: ${JSON.stringify({ mismatchedLinearSupport, rootMismatchedLinearSupport })}`);
    }
    let mismatchedLinearCompileRejected = false;
    try {
      aliasLinear.compile({ backend: "cpu", inputShape: [3] });
    } catch (err) {
      mismatchedLinearCompileRejected = String(err && err.message || err).includes("linear trace input shape");
    }
    if (!mismatchedLinearCompileRejected) throw new Error("expected JS mismatched linear compile to reject");
    ...
    if (
      ...
      throw new Error(`unexpected JS batched linear compile support: ${JSON.stringify(batchedLinearSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS batched linear requirements: ${JSON.stringify(requirements)}`);
      }
    ...
    if (
      ...
      throw new Error(`unexpected JS flatten-linear compile support: ${JSON.stringify(flattenLinearSupport)}`);
    }
    ...
    if (
      ...
      throw new Error("unexpected JS activation constructor aliases");
    }
    if (nn.relu().kind !== "relu") throw new Error("unexpected JS nn.relu factory alias");
    ...
    if (
      ...
      throw new Error(`unexpected JS standalone relu compile support: ${JSON.stringify(standaloneReluSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS standalone sigmoid compile support: ${JSON.stringify(standaloneSigmoidSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS unary-chain compile support: ${JSON.stringify(unaryChainSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS unary-chain compile evidence: ${JSON.stringify(unaryEvidence)}`);
      }
    ...
    if (
      ...
      throw new Error(`unexpected JS standalone logSoftmax compile support: ${JSON.stringify(standaloneLogSoftmaxSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS feature-axis sum compiler evidence: ${JSON.stringify(featureSumSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS batch-axis mean compiler evidence: ${JSON.stringify(batchMeanSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS batch-axis max compiler evidence: ${JSON.stringify(batchMaxSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS standalone logSoftmax WebGPU capabilities: ${JSON.stringify(webgpuLogSoftmaxCapabilities, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              throw new Error(`unexpected executable standalone logSoftmax WebGPU device-buffer session inspection: ${JSON.stringify(deviceSessionInfo, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
            }
    ...
    if (
      ...
      throw new Error(`unexpected JS classifier head compile support: ${JSON.stringify(classifierHeadSupport)}`);
    }
    ...
    try {
      if (classifierHeadProgram.inputShape().join(",") !== "2,2" || classifierHeadProgram.outputShape().join(",") !== "2,3") {
        throw new Error(`unexpected JS classifier head Program shapes: ${classifierHeadProgram.inputShape()} -> ${classifierHeadProgram.outputShape()}`);
      }
      ...
      try {
        ...
        if (classifierHeadSession.inputShape().join(",") !== "2,2" || classifierHeadSession.outputShape().join(",") !== "2,3") {
          throw new Error(`unexpected JS classifier head Session shapes: ${classifierHeadSession.inputShape()} -> ${classifierHeadSession.outputShape()}`);
        }
        if (
          ...
          throw new Error(`unexpected JS classifier head Session buffer layout: ${JSON.stringify(classifierSessionLayout)}`);
        }
        ...
        if (!(tensorOutput instanceof Tensor) || tensorOutput.shape.join(",") !== "2,3") {
          throw new Error(`unexpected JS classifier head stepTensor shape: ${tensorOutput.shape}`);
        }
        ...
        if (flatTensorOutput.shape.join(",") !== "6") {
          throw new Error(`unexpected JS classifier head explicit stepTensor shape: ${flatTensorOutput.shape}`);
        }
        ...
        if (shapedTensorOutput.shape.join(",") !== "2,3") {
          throw new Error(`unexpected JS classifier head output Tensor stepTensor shape: ${shapedTensorOutput.shape}`);
        }
        ...
        try {
          ...
          if (boundTensorOutput.shape.join(",") !== "2,3") {
            throw new Error(`unexpected JS classifier head bound output Tensor stepTensor shape: ${boundTensorOutput.shape}`);
          }
    ...
    if (
      ...
      throw new Error(`unexpected JS token classifier compile support: ${JSON.stringify(tokenClassifierSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS standalone LayerNorm compile support: ${JSON.stringify(standaloneLayerNormSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS batched RMSNorm compile support: ${JSON.stringify(batchedRmsNormSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS nested single-linear compile support: ${JSON.stringify(nestedLinearSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS nested single-linear compile evidence: ${JSON.stringify(nestedLinearEvidence)}`);
      }
      ...
      if (!nestedLinearCompatibility.compatible || nestedLinearProgram.acceptsModule(nestedLinear) !== true) {
        throw new Error(`expected JS nested single-linear Program to accept its source module: ${JSON.stringify(nestedLinearCompatibility)}`);
      }
      ...
      if (
        ...
        throw new Error(`expected JS nested single-linear Program to reject same-shape bare Linear: ${JSON.stringify(bareLinearCompatibility)}`);
      }
    ...
    if (
      ...
      throw new Error(`unexpected JS MLP compile support: ${JSON.stringify(mlpSupport)}`);
    }
    if (
      ...
      throw new Error(`unexpected JS root MLP compiler signatures: ${JSON.stringify({ rootMlpCompilerSignatures, compilerSignatures: mlpSupport.compilerSignatures })}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS derived MLP compiler signatures: ${JSON.stringify({ rootMlpDerivedCompilerSignatures, compilerSignatures: mlpSupport.compilerSignatures })}`);
    }
    if (
      !rootMlpIr ||
      ...
      throw new Error(`unexpected JS root MLP compiler artifacts: ${JSON.stringify({ rootMlpIr, rootMlpKernelPlan, rootMlpMemoryLayout, rootMlpBufferLayout, rootMlpInputShape, rootMlpOutputShape, rootMlpShapeConstraints, rootMlpParameterLayout })}`);
    }
    ...
    if (
      ...
      throw new Error(`expected JS nn input/output shape helpers to use KernelPlan shape constraints: ${JSON.stringify({ constraintsOnlyInputShape, constraintsOnlyOutputShape })}`);
    }
    const mlpHostOutput = mlpFrontend.forward(frontendInput);
    if (!(mlpHostOutput instanceof Tensor)) {
      throw new Error("expected JS MLP eager output to be a Tensor");
    }
    ...
    try {
      ...
      const mlpProgramDesc = mlpProgram.desc;
      ...
      let mlpDerivedEvidence;
      let mlpDerivedCompilerSignatures;
      try {
      ...
      const legacyConstructorProgram = new Program(mlpProgram.handle, mlpProgram.desc, {
        ...mlpEvidence,
        compilerSignatures: undefined,
        irSignature: undefined,
        kernelPlanSignature: undefined,
      ...
      if (
        ...
        throw new Error(`unexpected JS MLP requirements: ${JSON.stringify(mlpRequirements)}`);
      }
      ...
      if (
        ...
        throw new Error(`unexpected JS MLP module compatibility: ${JSON.stringify(mlpCompatibility)}`);
      }
      const staleFlatSignatureModule = {
        compileSupport() {
          return {
            ...mlpSupport,
      ...
      if (
        ...
        throw new Error(`expected JS canonical compilerSignatures to override stale flat aliases: ${JSON.stringify(staleFlatSignatureCompatibility)}`);
      }
      const irSignatureMismatchModule = {
        compileSupport() {
          return {
            ...mlpSupport,
            compilerSignatures: undefined,
      ...
      if (
        ...
        throw new Error(`expected JS IR signature compatibility mismatch: ${JSON.stringify(irSignatureMismatchCompatibility)}`);
      }
      const nestedIrSignatureMismatchModule = {
        compileSupport() {
          return {
            ...mlpSupport,
            irSignature: undefined,
      ...
      if (
        ...
        throw new Error(`expected JS nested IR signature compatibility mismatch: ${JSON.stringify(nestedIrSignatureMismatchCompatibility)}`);
      }
      const descriptorCountMismatchModule = {
        compileSupport() {
          return {
            ...mlpSupport,
            kernelPlan: {
      ...
      if (
        ...
        throw new Error(`expected JS descriptor-count compatibility mismatch: ${JSON.stringify(descriptorCountMismatchCompatibility)}`);
      }
      const memoryLayoutMismatchModule = {
        compileSupport() {
          return {
            ...mlpSupport,
      ...
      if (
        ...
        throw new Error(`expected JS memory-layout compatibility mismatch: ${JSON.stringify(memoryLayoutMismatchCompatibility)}`);
      }
      const parameterLayoutMismatchModule = {
        compileSupport() {
          return {
            ...mlpSupport,
            compilerSignatures: undefined,
      ...
      if (
        ...
        throw new Error(`expected JS parameter-layout compatibility mismatch: ${JSON.stringify(parameterLayoutMismatchCompatibility)}`);
      }
      const bufferLayoutMismatchModule = {
        compileSupport() {
          return {
            ...mlpSupport,
            compilerSignatures: undefined,
      ...
      if (
        ...
        throw new Error(`expected JS buffer-layout compatibility mismatch: ${JSON.stringify(bufferLayoutMismatchCompatibility)}`);
      }
      ...
      if (
        ...
        throw new Error(`expected incompatible JS MLP module compatibility: ${JSON.stringify(incompatibleMlpCompatibility)}`);
      }
      let incompatibleBindRejected = false;
      try {
        mlpProgram.bindModule(incompatibleMlp);
      } catch (err) {
        incompatibleBindRejected = String(err && err.message || err).includes("does not match");
      }
      if (!incompatibleBindRejected) throw new Error("expected JS Program.bindModule to reject incompatible module");
      let incompatiblePlacementRejected = false;
      let incompatiblePlacementBindings = null;
      try {
        ...
        if (incompatiblePlacementBindings) {
          if (incompatiblePlacementBindings.weights instanceof NativeBuffer) incompatiblePlacementBindings.weights.free();
          if (incompatiblePlacementBindings.bias instanceof NativeBuffer) incompatiblePlacementBindings.bias.free();
        }
      }
      if (!incompatiblePlacementRejected) throw new Error("expected JS nn.placeParameters to reject incompatible module before placement");
      ...
      try {
        if (!(placedMlpBindings.weights instanceof NativeBuffer) || !(placedMlpBindings.bias instanceof NativeBuffer)) {
          throw new Error("expected JS module.placeParameters to return NativeBuffer parameter bindings");
        }
    ...
    if (
      ...
      throw new Error(`unexpected JS nested MLP compile support: ${JSON.stringify(nestedMlpSupport)}`);
    }
    ...
    if (!siluNoBiasSupport.supported || siluNoBiasSupport.modelKind !== "module") {
      throw new Error(`unexpected JS no-bias SiLU MLP compile support: ${JSON.stringify(siluNoBiasSupport)}`);
    }
    ...
    try {
      ...
      if (siluNoBiasRequirements.modelKind !== "module" || siluNoBiasRequirements.weightsLen !== 6 || siluNoBiasRequirements.biasLen !== 0) {
        throw new Error(`unexpected JS no-bias SiLU MLP requirements: ${JSON.stringify(siluNoBiasRequirements)}`);
      }
    ...
    if (
      ...
      throw new Error(`unexpected JS softmax compile support: ${JSON.stringify(softmaxSupport)}`);
    }
    ...
    if (!(lnOut instanceof Tensor)) {
      throw new Error("expected JS layerNorm Tensor input to return a Tensor");
    }
    ...
    if (!(rmsOut instanceof Tensor)) {
      throw new Error("expected JS rmsNorm Tensor input to return a Tensor");
    }
    ...
    if (!(embeddingOut instanceof Tensor) || embeddingOut.shape.join(",") !== "3,3") {
      throw new Error("expected JS embedding output to be a rank-2 Tensor");
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS embedding compile support: ${JSON.stringify(embeddingSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS shaped embedding compile support: ${JSON.stringify(shapedEmbeddingSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS root embedding compile helpers: ${JSON.stringify({ rootEmbeddingSupport, rootEmbeddingTrace })}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS embedding module requirements: ${JSON.stringify(requirements)}`);
      }
      ...
      if (!defaultEmbeddingCompatibility.compatible || embeddingProgram.acceptsModule(embedding) !== true) {
        throw new Error(`expected embedding Program evidence to supply default module inputShape: ${JSON.stringify(defaultEmbeddingCompatibility)}`);
      }
      ...
      try {
        expectClose(
          ...
          nn.bindParameters(embedding, { inputShape: [3] }).weights,
        );
      } finally {
    ...
    try {
      ...
      rootEmbeddingCompileRejected = String(err && err.message || err).includes("requires inputShape");
    }
    if (!rootEmbeddingCompileRejected) throw new Error("expected JS root nn.compile to reject unshaped embedding");
    ...
    if (tokenParams.length !== 1 || tokenParams[0].name !== "0.weight") {
      throw new Error(`unexpected JS embedding parameter list: ${JSON.stringify(tokenParams.map((param) => param.name))}`);
    }
    if (!(tokenParams[0].grad instanceof Float32Array) || tokenParams[0].grad.length !== tokenParams[0].data.length) {
      throw new Error("JS embedding parameter did not expose a durable gradient buffer");
    }
    const tokenFrontendOut = tokenFrontend.forward([1]);
    if (!(tokenFrontendOut instanceof Tensor)) {
      throw new Error("expected JS embedding sequential output to be a Tensor");
    }
    expectCloseWithin(
    ...
    if (
      ...
      throw new Error(`unexpected JS embedding Sequential compile support: ${JSON.stringify(tokenFrontendSupport)}`);
    }
    ...
    if (!(ceLoss instanceof Tensor) || Math.abs(ceLoss.item() - ceExpected) > 1e-6) {
      throw new Error("expected JS crossEntropy over Tensor logits to return a scalar Tensor loss");
    }
    ...
    if (!(classifierLoss instanceof Tensor)) {
      throw new Error("expected JS classifier crossEntropy loss to be a Tensor");
    }
    ...
    if (!(autogradLoss instanceof Tensor) || Math.abs(autogradLoss.item() - 1) > 1e-6) {
      throw new Error("expected JS MSE over tensors to return a scalar Tensor loss");
    }
    ...
    train.step(optim.sgd(autogradStep, { lr: 0.1 }), {
      loss: loss.mse(autogradStep.forward(tensor([2, -1], [2])), tensor([1], [1])),
    });
    ...
    if (momentumState.kind !== "sgd" || momentumState.paramCount !== 2 || momentumState.entries.length !== 2) {
      throw new Error(`unexpected JS SGD optimizer state: ${JSON.stringify(momentumState)}`);
    }
    if (momentumState.entries[0].name !== "velocity.0" || momentumState.entries[0].layout !== "row-major:linear.weight[in_features,out_features]") {
      throw new Error(`unexpected JS SGD optimizer state entry: ${JSON.stringify(momentumState.entries[0])}`);
    }
    ...
    try {
      ...
      optimizerLayoutRejected = String(err && err.message || err).includes("layout must be row-major:linear.weight[in_features,out_features]");
    }
    if (!optimizerLayoutRejected) throw new Error("expected JS optimizer state layout rejection");
    ...
    if (adamwState.kind !== "adamw" || adamwState.step !== 1 || adamwState.entries.length !== 4) {
      throw new Error(`unexpected JS AdamW optimizer state: ${JSON.stringify(adamwState)}`);
    }
    ...
    if (!(trainable.weight[0] < 0.975)) {
      throw new Error("expected JS AdamW optimizer to update module parameters");
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS multi-layer compile support: ${JSON.stringify(multiLayerSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS multi-layer module requirements: ${JSON.stringify(requirements)}`);
      }
      ...
      try {
        ...
        if (sessionInfo.modelKind !== "module" || sessionInfo.persistentBindingCount !== 1) {
          throw new Error(`unexpected JS multi-layer module session inspection: ${JSON.stringify(sessionInfo)}`);
        }
      } finally {
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            throw new Error(`unexpected JS native-buffer module session inspection: ${JSON.stringify(nativeInfo)}`);
          }
          const eager = multiLayer.forward(frontendInput);
    ...
    if (
      ...
      throw new Error(`unexpected JS batched module compile support: ${JSON.stringify(batchedMultiLayerSupport)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected JS batched module requirements: ${JSON.stringify(requirements)}`);
      }
      ...
      if (!batchedCompatibility.compatible || batchedMultiLayerProgram.acceptsModule(multiLayer) !== true) {
        throw new Error(`expected batched Program evidence to supply default module inputShape: ${JSON.stringify(batchedCompatibility)}`);
      }
    ...
    if (
      ...
      throw new Error(`unexpected JS nested module compile support: ${JSON.stringify(nestedModuleSupport)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS nested module trace: ${JSON.stringify(nestedModuleTrace)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected JS shaped nested module trace: ${JSON.stringify(nestedModuleShapedTrace)}`);
    }
    let traceShapeRejected = false;
    try {
      nestedModule.trace({ inputShape: [3] });
    } catch (err) {
      traceShapeRejected = String(err && err.message || err).includes("linear trace input shape");
    }
    if (!traceShapeRejected) throw new Error("expected JS trace shape mismatch rejection");
    ...
    try {
      ...
      try {
        bound.readOutputInto(new Uint8Array(3));
      } catch (err) {
        hostReadTargetRejected = String(err && err.message || err).includes("Float32Array target");
      }
      ...
      try {
        ...
        hostReadAlignmentRejected = String(err && err.message || err).includes("aligned to f32");
      }
      ...
      try {
        ...
        hostReadRangeRejected = String(err && err.message || err).includes("source range is out of bounds");
      }
    ...
    if (
      ...
      throw new Error(`unexpected host output inspection: ${JSON.stringify(factoryOutputInfo)}`);
    }
    ...
    if (!Object.isFrozen(nativeOutputInfo) || nativeOutputInfo.storage !== "host" || nativeOutputInfo.byteLength !== nativeOutput.byteLength) {
      throw new Error(`unexpected native output inspection: ${JSON.stringify(nativeOutputInfo)}`);
    }
    ...
    try {
      ...
      try {
        ...
        if (!String(err && err.message || err).includes("exceeds buffer byteLength")) throw err;
      }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected tiny linear session inspection: ${JSON.stringify(nativeBoundInfo)}`);
      }
      ...
      try {
        ...
        if (!String(err && err.message || err).includes("shape_mismatch")) throw err;
      }
    ...
    try {
      ...
      mismatchedPlacementRejected = String(err && err.message || err).includes("Program input slot length 2");
    }
    ...
    const resourceWeights = program.createBuffer("weights", {
      resource: (slot) => {
        if (
          ...
          throw new Error(`unexpected weights resource slot: ${JSON.stringify(slot)}`);
        }
    ...
    const resourceOutput = program.createBuffer("output", {
      resource: (slot) => {
        if (
          ...
          throw new Error(`unexpected output resource slot: ${JSON.stringify(slot)}`);
        }
        resourceSlotAudit.push(`${slot.kind}:${slot.role}:${slot.elementCount}:${slot.byteLength}`);
        return NativeBuffer.externalResource({
        placement: "webgpu",
    ...
    const resourceInput = program.createBuffer("input", {
      resource: (slot) => {
        if (
          ...
          throw new Error(`unexpected input resource slot: ${JSON.stringify(slot)}`);
        }
        resourceSlotAudit.push(`${slot.kind}:${slot.role}:${slot.elementCount}:${slot.byteLength}`);
        return NativeBuffer.externalResource({
        placement: "webgpu",
    ...
    try {
      ...
      if (
        ...
        !resourceWeightsInfo.access.read ||
        resourceWeightsInfo.access.write ||
        ...
        throw new Error(`unexpected external weights inspection: ${JSON.stringify(resourceWeightsInfo)}`);
      }
      ...
      if (
        ...
        resourceOutputInfo.access.read ||
        !resourceOutputInfo.access.write ||
        ...
        throw new Error(`unexpected external output inspection: ${JSON.stringify(resourceOutputInfo)}`);
      }
      ...
      if (
        ...
        !resourceInputInfo.access.read ||
        resourceInputInfo.access.write ||
        resourceInputInfo.handle !== 3 ||
        resourceInputInfo.byteLength !== requirements.inputLen * requirements.scalarBytes
      ) {
        throw new Error(`unexpected external input inspection: ${JSON.stringify(resourceInputInfo)}`);
      }
  ...
  try {
    ...
    if (
      ...
      throw new Error(`unexpected tiny linear WebGPU inspection: ${JSON.stringify(webgpuInspection, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
    if (runtimeReportsNativeWgpu) {
      if (
        ...
        throw new Error(`unexpected native tiny linear WebGPU dispatch plan: ${JSON.stringify(webgpuInspection, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
    } else if (
      webgpuInspection.backendDispatchCount !== 0 ||
      webgpuInspection.dispatchPlanSupported ||
      ...
      throw new Error(`unexpected compile-only tiny linear WebGPU dispatch plan: ${JSON.stringify(webgpuInspection, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
    if (
      ...
      throw new Error(`unexpected WebGPU tiny linear capabilities: ${JSON.stringify(webgpuCapabilities, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
      ...
      try {
        ...
        try {
          ...
          if (noOutputProfile.callCount !== 1 || noOutputProfile.syncCount !== 0) {
            throw new Error(`expected no-readback tiny linear WebGPU advance profile, got ${JSON.stringify(noOutputProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
          }
        } finally {
          nativeWebgpu.free();
        }
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            throw new Error(`unexpected executable tiny linear WebGPU device-buffer session inspection: ${JSON.stringify(deviceInfo, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
          }
          ...
          if (deviceNoOutputProfile.callCount !== 1 || deviceNoOutputProfile.syncCount !== 0) {
            throw new Error(`expected no-readback tiny linear WebGPU device-buffer advance profile, got ${JSON.stringify(deviceNoOutputProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
          }
        } finally {
          deviceWebgpu.free();
        }
        ...
        if (webgpuDevice.handle !== deviceHandle || webgpuDevice.placement !== "webgpu") {
          throw new Error(`unexpected tiny linear WebGPU ProgramDevice: ${JSON.stringify({ handle: webgpuDevice.handle, placement: webgpuDevice.placement })}`);
        }
        ...
        try {
          ...
          fakeObjectImportRejected = String(err.message || err).includes("zgml-created device buffer");
        } finally {
      ...
      if (webgpuInspection.executionSupported) {
        ...
        try {
          ...
          if (
            ...
            throw new Error(`unexpected tiny linear WebGPU resource session inspection: ${JSON.stringify(sessionInfo)}`);
          }
          ...
          if (
            ...
            throw new Error(`tiny linear WebGPU resource probe recorded runtime work: ${JSON.stringify(webgpuResourceProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
          }
        } finally {
          webgpuSession.free();
        }
  ...
  try {
    ...
    if (
      ...
      throw new Error(`unexpected traced MLP WebGPU inspection: ${JSON.stringify(webgpuMlpInspection, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected WebGPU traced MLP capabilities: ${JSON.stringify(webgpuMlpCapabilities, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
    ...
    if (webgpuMlpInspection.executionSupported) {
      ...
      try {
        ...
        if (
          ...
          throw new Error(`unexpected executable traced MLP WebGPU session inspection: ${JSON.stringify(sessionInfo, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
        }
      } finally {
...
try {
  ...
  if (
    ...
    throw new Error(`unexpected tiny llama model inspection: ${JSON.stringify(modelInspection)}`);
  }
  ...
  try {
    ...
    unsupportedBatchRejected =
      err &&
      (err.status === "unsupported" || err.code === 5 || String(err.message).includes("unsupported"));
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error(`unexpected tiny LLaMA requirements: ${JSON.stringify(requirements)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected tiny LLaMA buffer layout: ${JSON.stringify(bufferLayout)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected llama executable inspection: ${JSON.stringify(executable)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected llama executable capabilities: ${JSON.stringify(executableCapabilities, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected tiny LLaMA session inspection: ${JSON.stringify(sessionInfo)}`);
    }
    ...
    try {
      program.bind({ output: boundOutput });
    } catch (err) {
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected compatible LLaMA preflight: ${JSON.stringify(compatible)}`);
      }
      ...
      if (
        ...
        throw new Error(`unexpected incompatible LLaMA preflight: ${JSON.stringify(incompatible)}`);
      }
    } finally {
    ...
    if (!Object.isFrozen(kvReq) || kvReq.modelKind !== "tiny-llama" || kvReq.scalarBytes !== 4 || kvReq.layers !== 1 || kvReq.contextLength !== inspection.contextLen || kvReq.bufferByteLength !== 64) {
      throw new Error(`unexpected tiny LLaMA KV cache requirements: ${JSON.stringify(kvReq)}`);
    }
    ...
    if (
      ...
      throw new Error(`unexpected tiny LLaMA KV cache layout: ${JSON.stringify(kvLayout)}`);
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected host-KV LLaMA Session KV layout: ${JSON.stringify(hostSessionKvLayout)}`);
      }
      ...
      if (hostKvSession) hostKvSession.free();
      hostKvCache.free();
    }
    ...
    const kvCache = program.createKvCache({
      resource: (slot) => {
        if (
          ...
          throw new Error(`unexpected KV resource slot: ${JSON.stringify(slot)}`);
        }
    ...
    try {
      ...
      if (
        ...
        throw new Error(`unexpected bound LLaMA Session buffer layout: ${JSON.stringify(sessionLayout)}`);
      }
      ...
      if (
        ...
        throw new Error(`unexpected bound LLaMA Session KV layout: ${JSON.stringify(sessionKvLayout)}`);
      }
      const outA = new Float32Array(TinyLlamaSessionVocabPlusSentinel);
      outA.fill(-999);
      const logitsA = a.step(0, outA);
      if (logitsA.length !== 8 || a.position() !== 1 || b.position() !== 0) {
        throw new Error(`unexpected llama step state: logits=${logitsA.length} a=${a.position()} b=${b.position()}`);
      }
      ...
      const windowStepScalarExecuteOptionsScratch = windowStep.scalarTokenExecuteOptionsScratch;
      const windowStepOutputOptionsScratch = windowStep.outputTokenWindowOptionsScratch;
      const originalWindowStepExecuteTokens = windowStep.executeTokens;
      const capturedWindowStepExecutes = [];
      windowStep.executeTokens = function captureWindowStepExecute(tokens, options = {}) {
      ...
      try {
        ...
        windowStep.executeTokens = originalWindowStepExecuteTokens;
      }
      windowStep.reset();
      ...
      if (
        ...
        throw new Error(`expected cold LLaMA program profile, got ${JSON.stringify(llamaProgramProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
      ...
      if (
        ...
        throw new Error(`unexpected LLaMA session profile: ${JSON.stringify(llamaSessionProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
      ...
      if (
        ...
        throw new Error(`unexpected reset LLaMA session profile: ${JSON.stringify(resetLlamaProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
      ...
      const scalarScratch = factoryBound.scalarTokenScratch;
      ...
      const scalarWindowOptionsScratch = factoryBound.scalarTokenWindowOptionsScratch;
      const scalarSampleOptionsScratch = factoryBound.scalarTokenSampleOptionsScratch;
      const noOutputWindowOptionsScratch = factoryBound.noOutputTokenWindowOptionsScratch;
      if (
      ...
      const originalExecuteTokens = factoryBound.executeTokens;
      const originalExecuteTokensArgmax = factoryBound.executeTokensArgmax;
      const originalExecuteTokensSample = factoryBound.executeTokensSample;
      let capturedScalarAdvance = null;
      let capturedScalarArgmax = null;
      let capturedScalarSample = null;
      try {
        factoryBound.executeTokens = function captureScalarExecute(tokens, options = {}) {
          capturedScalarAdvance = { tokens, options };
          return originalExecuteTokens.call(this, tokens, options);
        };
        factoryBound.executeTokensArgmax = function captureScalarArgmax(tokens, options = {}) {
          capturedScalarArgmax = { tokens, options };
          return originalExecuteTokensArgmax.call(this, tokens, options);
        };
        factoryBound.executeTokensSample = function captureScalarSample(tokens, options = {}) {
        ...
        if (
          ...
          throw new Error(`native scalar helpers did not reuse one-token scratch: ${JSON.stringify({
        ...
        factoryBound.executeTokens = originalExecuteTokens;
        factoryBound.executeTokensArgmax = originalExecuteTokensArgmax;
        factoryBound.executeTokensSample = originalExecuteTokensSample;
      }
      nativeBound.reset();
      ...
      if (borrowedArgmax.token !== expectedArgmax.token || borrowedArgmax.logit !== expectedArgmax.logit || factoryBound.position() !== 1) {
        throw new Error(`unexpected borrowed argmax token window: ${JSON.stringify({ borrowedArgmax, position: factoryBound.position() })}`);
      }
      factoryBound.reset();
      ...
      if (borrowedSample.token !== expectedFirst.token || borrowedSample.logit !== expectedFirst.logit || factoryBound.position() !== 1) {
        throw new Error(`unexpected borrowed sampled token window: ${JSON.stringify({ borrowedSample, position: factoryBound.position() })}`);
      }
      factoryBound.reset();
      ...
      const originalBorrowedExecuteTokens = factoryBound.executeTokens;
      let capturedBorrowedAdvance = null;
      try {
        factoryBound.executeTokens = function captureBorrowedAdvance(tokens, options = {}) {
        ...
        factoryBound.executeTokens = originalBorrowedExecuteTokens;
      }
      ...
      if (
        ...
        throw new Error(`unexpected native generated sample: ${JSON.stringify({
      ...
      if (
        ...
        throw new Error(`unexpected native generated sample into: ${JSON.stringify({
      ...
      if (
        ...
        throw new Error(`unexpected borrowed native generated sample: ${JSON.stringify({
      ...
      if (
        ...
        throw new Error(`unexpected native generated argmax: ${JSON.stringify({
      ...
      if (
        ...
        throw new Error(`unexpected native generated argmax into: ${JSON.stringify({
      ...
      if (
        ...
        throw new Error(`unexpected borrowed native generated argmax: ${JSON.stringify({
      ...
      if (
        ...
        throw new Error(`unexpected auto-native generated argmax: ${JSON.stringify({
      ...
      if (logitsAdvanced.length !== 8 || advanced.position() !== 2 || outAdvanced[8] !== -333) {
      ...
      try {
        ...
        tooLongRejected = err && err.status === "shape_mismatch";
      }
      ...
      try {
        ...
        tooLongPrefillRejected = err && err.status === "shape_mismatch";
      }
      ...
      if (logitsBulkAdvanced.length !== 8 || bulkAdvanced.position() !== 3 || outBulkAdvanced[8] !== -444) {
      ...
      if (logitsBulkExpected.length !== 8 || bulkExpected.position() !== 3 || outBulkExpected[8] !== -123) {
      ...
      if (logitsPrefill.length !== 8 || prefilled.position() !== 2 || outPrefill[8] !== -222) {
      ...
      console.log(`zgml node ffi tiny llama smoke ok: ${logitsA.length} logits`);
    } finally {
...
try {
  ...
  try {
    ...
    if (
      ...
      throw new Error(`unexpected WebGPU LLaMA inspection: ${JSON.stringify({ inspection, executable }, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
    if (llamaWebgpuExecutable) {
      if (
        ...
        throw new Error(`unexpected executable WebGPU LLaMA dispatch plan: ${JSON.stringify(executable, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
      }
    } else if (
      executable.backendDispatchCount !== 0 ||
      executable.dispatchPlanSupported ||
      ...
      throw new Error(`unexpected WebGPU compile-only LLaMA dispatch plan: ${JSON.stringify(executable, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
    if (
      ...
      throw new Error(`unexpected WebGPU LLaMA capabilities: ${JSON.stringify(capabilities, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    }
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            throw new Error(`unexpected executable WebGPU LLaMA session profile: ${JSON.stringify(webgpuProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
          }
          ...
          try {
            ...
            if (
              ...
              throw new Error(`unexpected executable WebGPU LLaMA no-output profile: ${JSON.stringify(noOutputProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
            }
          } finally {
          ...
          let deviceKvCache = null;
          let importedDeviceKvCache = null;
          try {
            ...
            if (
              ...
              throw new Error(`unexpected executable WebGPU LLaMA ProgramDevice KV cache layout: ${JSON.stringify({
            ...
            try {
              ...
              if (
                ...
                throw new Error(`unexpected executable WebGPU LLaMA resource step profile: ${JSON.stringify(resourceProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
              }
              ...
              if (
                ...
                throw new Error(`unexpected executable WebGPU LLaMA resource session inspection: ${JSON.stringify(resourceInfo, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
              }
            } finally {
              webgpuResourceSession.free();
            ...
            try {
              ...
              if (
                ...
                throw new Error(`unexpected executable WebGPU LLaMA resource argmax: ${JSON.stringify({ selected, expectedSelect, position: webgpuSelectSession.position() })}`);
              }
            } finally {
            ...
            try {
              ...
              const cpuSampleLogits = cpuProgram.bind();
              try {
                const expectedSample = argmax(cpuSampleLogits.step(0, new Float32Array(TinyLlamaSessionVocabPlusSentinel).fill(-1616)));
                if (
                  sampled.token !== expectedSample.token ||
                  Math.abs(sampled.logit - expectedSample.logit) > 1e-3 ||
                  webgpuSampleSession.position() !== 1
                ) {
                  throw new Error(`unexpected executable WebGPU LLaMA resource top-k sample: ${JSON.stringify({ sampled, expectedSample, position: webgpuSampleSession.position() })}`);
                }
              } finally {
                cpuSampleLogits.free();
              }
            } finally {
            ...
            try {
              ...
              if (
                ...
                throw new Error(`unexpected executable WebGPU LLaMA resource greedy generation: ${JSON.stringify({ generated, expectedFirst, expectedSecond, position: webgpuGenerateSession.position() })}`);
              }
            } finally {
            ...
            try {
              ...
              try {
                ...
                if (
                  ...
                  throw new Error(`unexpected executable WebGPU LLaMA resource sampled generation: ${JSON.stringify({ generatedSample, expectedFirst, expectedSecond, position: webgpuGenerateSampleSession.position() })}`);
                }
              } finally {
            ...
            try {
              ...
              if (
                ...
                throw new Error(`unexpected executable WebGPU LLaMA resource no-output profile: ${JSON.stringify(noOutputResourceProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
              }
            ...
            try {
              ...
              if (
                ...
                throw new Error(`unexpected executable WebGPU LLaMA over-context fill profile: ${JSON.stringify(beforeOverContextReject, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
              }
              ...
              if (
                ...
                throw new Error(`expected executable WebGPU LLaMA over-context resource greedy generation rejection without state mutation, pos=${webgpuOverContextSession.position()} tokens=${Array.from(overContextGreedyTokens)}`);
              }
              expectCloseWithin(
                deviceOutput.readFloat32(TinyLlamaSessionVocabPlusSentinel - 1),
                Array(TinyLlamaSessionVocabPlusSentinel - 1).fill(-8181),
              ...
              if (
                ...
                throw new Error(`expected executable WebGPU LLaMA over-context resource sampled generation rejection without state mutation, pos=${webgpuOverContextSession.position()} tokens=${Array.from(overContextSampleTokens)}`);
              }
              expectCloseWithin(
                deviceOutput.readFloat32(TinyLlamaSessionVocabPlusSentinel - 1),
                Array(TinyLlamaSessionVocabPlusSentinel - 1).fill(-8181),
            ...
            if (importedDeviceKvCache) {
              for (const buffer of [...importedDeviceKvCache.k, ...importedDeviceKvCache.v]) buffer.free();
            }
            importedDeviceOutput.free();
            if (deviceKvCache) deviceKvCache.free();
            deviceOutput.free();
          }
        } finally {
          webgpuSession.free();
      ...
      if (llamaWebgpuExecutable) {
        ...
        try {
          ...
          if (
            ...
            throw new Error(`unexpected WebGPU LLaMA resource session inspection: ${JSON.stringify(sessionInfo)}`);
          }
          ...
          if (
            ...
            throw new Error(`WebGPU LLaMA unsupported step recorded runtime work: ${JSON.stringify(resourceProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
          }
        } finally {
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              throw new Error(`unexpected model-bound WebGPU LLaMA resource session inspection: ${JSON.stringify(modelSessionInfo)}`);
            }
            ...
            if (
              ...
              throw new Error(`model-bound WebGPU LLaMA unsupported step recorded runtime work: ${JSON.stringify(modelResourceProfile, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
            }
          } finally {
    ...
    console.log(`zgml node ffi ${llamaWebgpuExecutable ? "executable" : "resource-probe"} webgpu tiny llama smoke ok`);
  } finally {
    program.free();
  }
} finally {
...
if (smollmPath) {
  const resolvedSmollmPath = path.isAbsolute(smollmPath)
    ? smollmPath
    : path.join(process.env.INIT_CWD || process.cwd(), smollmPath);
  const autoProbe = probeModel(resolvedSmollmPath);
  if (
    ...
    throw new Error(`unexpected auto LLaMA model probe: ${JSON.stringify(autoProbe)}`);
  }
  const fixedProbe = SmolLM135M.probe(resolvedSmollmPath);
  if (fixedProbe.modelKind !== autoProbe.modelKind || fixedProbe.vocabSize !== autoProbe.vocabSize) {
    throw new Error(`unexpected fixed SmolLM model probe: ${JSON.stringify(fixedProbe)}`);
  }
  const model = loadModel(resolvedSmollmPath);
  try {
    const modelInspection = model.inspect();
    if (
      ...
      throw new Error(`unexpected auto LLaMA model inspection: ${JSON.stringify(modelInspection)}`);
    }

    const program = model.compile({ backend: "cpu", contextLength: 16 });
    try {
      const inspection = program.inspect();
      if (
      ...
      if (
        ...
        executable.bufferByteLength === 0 ||
        executable.opCount === 0 ||
        executable.runtimePatchMaxCacheWritePos !== 15 ||
        executable.runtimePatchMaxAttentionSeqKv !== 16 ||
        executable.commandStencilHash === 0n ||
        (executable.commandProjectionCount === 0 && executable.commandAttentionCount === 0) ||
        executable.runtimePatchStencilHash === 0n ||
        executable.runtimePatchHoles !== inspection.semanticRuntimePatchHoles ||
        ...
        throw new Error(`unexpected auto LLaMA executable inspection: ${JSON.stringify(executable)}`);
      }
      ...
      try {
        if (session.vocabSize !== inspection.vocabSize || prefillSession.vocabSize !== inspection.vocabSize) {
          throw new Error(`unexpected auto LLaMA session vocab sizes: step=${session.vocabSize} prefill=${prefillSession.vocabSize}`);
        }
        ...
        if (out[inspection.vocabSize] !== -555) {
          throw new Error("auto LLaMA step wrote past vocab logits");
        }
        session.reset();
        ...
        if (prefillLogits.length !== inspection.vocabSize || prefillSession.position() !== 2) {
          throw new Error(`unexpected auto LLaMA prefill: logits=${prefillLogits.length} pos=${prefillSession.position()}`);
        }
        if (prefillOut[inspection.vocabSize] !== -444) {
          throw new Error("auto LLaMA prefill wrote past vocab logits");
        }
        console.log(`zgml node ffi auto llama smoke ok: ${logits.length} logits`);
      } finally {
        prefillSession.free();
        session.free();
      }
  ...
  const smollm = loadModel(resolvedSmollmPath, { kind: "smollm-135m" });
  try {
    ...
    if (
      ...
      throw new Error(`unexpected SmolLM model inspection: ${JSON.stringify(modelInspection)}`);
    }

    const program = smollm.compile({ backend: "cpu", contextLength: 16 });
    try {
      const inspection = program.inspect();
      if (
        inspection.vocabSize !== 49152 ||
      ...
      if (
        ...
        throw new Error(`unexpected SmolLM executable inspection: ${JSON.stringify(executable)}`);
      }
      ...
      try {
        ...
        const logits = session.step(0, out);
        if (logits.length !== inspection.vocabSize || session.position() !== 1) {
          throw new Error(`unexpected SmolLM step: logits=${logits.length} pos=${session.position()}`);
        }
        if (out[inspection.vocabSize] !== -333) {
          throw new Error("SmolLM step wrote past vocab logits");
        }
        ...
        const prefillLogits = prefillSession.prefill([0, 1], prefillOut);
        if (prefillLogits.length !== inspection.vocabSize || prefillSession.position() !== 2) {
          throw new Error(`unexpected SmolLM prefill: logits=${prefillLogits.length} pos=${prefillSession.position()}`);
        }
        if (prefillOut[inspection.vocabSize] !== -222) {
          throw new Error("SmolLM prefill wrote past vocab logits");
        }
        console.log(`zgml node ffi SmolLM smoke ok: ${logits.length} logits`);
      } finally {
        prefillSession.free();
        session.free();
      }
