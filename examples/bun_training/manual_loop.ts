...
class RegisteredLinearHead extends nn.Module<readonly [1], readonly [1]> {
  declare head: ReturnType<typeof nn.linear<1, 1>>;
...
class RegisteredParameterHead extends nn.Module<readonly [1], readonly [1]> {
  declare weight: ReturnType<typeof nn.Parameter<readonly [1]>>;
  declare bias: ReturnType<typeof nn.Parameter<readonly [1]>>;
...
const fieldParameterLoss = loss.mse(fieldParameterModel.forward(tensor([2], [1] as const)), tensor([3], [1] as const));
...
const registeredLinearProgram = registeredLinear.compile({ backend: "cpu", inputShape: [1] as const });
const registeredLinearSession = registeredLinearProgram.bindModule(registeredLinear);
const registeredLinearInput = tensor([3], [1] as const);
...
const registeredParameterLoss = loss.mse(registeredParameterModel.forward(tensor([2], [1] as const)), tensor([3], [1] as const));
...
const moduleListApplyNames: string[] = [];
...
const moduleListProgram = moduleListModel.compile({ backend: "cpu", inputShape: [1] as const });
const moduleListSession = moduleListProgram.bindModule(moduleListModel);
const moduleListInput = tensor([4], [1] as const);
...
const parameterListLoss = loss.mse(parameterListModel.forward(tensor([2], [1] as const)), tensor([3], [1] as const));
...
const moduleDictApplyNames: string[] = [];
...
const moduleDictProgram = moduleDictModel.compile({ backend: "cpu", inputShape: [1] as const });
const moduleDictSession = moduleDictProgram.bindModule(moduleDictModel);
const moduleDictInput = tensor([5], [1] as const);
...
const parameterDictLoss = loss.mse(parameterDictModel.forward(tensor([2], [1] as const)), tensor([3], [1] as const));
...
const layerList = [nn.linear(2, 2), nn.relu()] as const;
const layerListApplyNames: string[] = [];
...
console.log(`zgml bun manual training loop smoke ok: before=${before.toFixed(6)} after=${after.toFixed(6)} manualSteps=${manualSteps} compiled=${scalar(compiled).toFixed(6)}`);
