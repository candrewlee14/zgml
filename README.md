...
factories (`zeros`, `ones`, `full`, `rand`, `randn`, `linspace`, `arange`,
  ...
  differentiable tensor joins (`cat`, `concat`, `concatenate`, `stack`, `vstack`, `hstack`) and eager/autograd `einsum` with ellipsis, broadcast semantics, and literal-equation shape inference,
...
eager/autograd `einsum` semantics with typed output shapes rather than only API
exports.
