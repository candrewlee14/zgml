...
`goal progress: Program/Session substrate=71% floor=65%; PyTorch-like surface=78% floor=60%`.
...
native Program construction, and a benchmarked allocation-free lazy
`matmul -> add -> relu` Session path plus a direct lazy MLP Session path rather
than only API exports.
