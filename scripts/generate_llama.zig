//! Generate text from a pretrained HuggingFace LLaMA-compatible model.
//!
//! Loads weights from safetensors, tokenizes a prompt with GPT-2 BPE,
//! and generates tokens with zero per-step allocation.
//!
//! Build: zig build generate-llama
//! Run:   ./zig-out/bin/generate-llama data/smollm/model.safetensors \
//!            data/smollm/vocab.json data/smollm/merges.txt "Once upon a time"
...
pub fn main(init: std.process.Init) !void {
    ...
    // Create session, then load weights.
    try stderr.interface.print("Loading model...\n", .{});
    stderr.interface.flush() catch {};
    ...
    for (0..prompt_ids.len + 200) |_| {
        ...
        // EOS token for SmolLM is 0 (<|endoftext|>)
        if (best == 0 or session.position() >= config.max_seq_len) break;
