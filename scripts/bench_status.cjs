...
const nativeEagerExpectedKeys = [
  ...
  "elementwise_div_batched",
  "elementwise_minimum_batched",
  "elementwise_maximum_batched",
  "elementwise_neg_batched",
  "elementwise_abs_batched",
  ...
  "elementwise_sub_lhs_row_broadcast_batched",
  "dot_batched",
  "activation_relu_batched",
  "activation_sigmoid_batched",
  "activation_gelu_batched",
  "activation_silu_batched",
  "activation_tanh_batched",
...
function formatSemanticSingleDispatchRefusals(refusals) {
  if (!refusals || typeof refusals !== "object") return "n/a";
  return semanticSingleDispatchRefusalReasons
    .map((reason) => `${reason}:${refusals[reason] ?? 0}`)
    .join(",");
}

function formatSemanticSingleDispatchDimRefusalShape(shape) {
  if (!shape || typeof shape !== "object") return "n/a";
  const k = Number(shape.k);
  const h = Number(shape.h);
  const o = Number(shape.o);
  const cap = Number(shape.cap);
  if (!Number.isFinite(k) || !Number.isFinite(h) || !Number.isFinite(o) || !Number.isFinite(cap)) return "n/a";
  return `k:${formatNumber(k, 0)},h:${formatNumber(h, 0)},o:${formatNumber(o, 0)},cap:${formatNumber(cap, 0)}`;
}
...
function q8PromptFreshnessStatusLine(selectedPath, rawPath) {
  ...
  try {
    ...
    const semanticDirectCount = data?.lanes?.semantic?.semanticWithInputDirectCount ?? "n/a";
    const semanticDirectRows = data?.lanes?.semantic?.semanticWithInputDirectRows ?? "n/a";
    const semanticDirectRowThreadgroups = data?.lanes?.semantic?.semanticWithInputDirectRowThreadgroups ?? "n/a";
    const semanticDirectRowSerialDotOps = data?.lanes?.semantic?.semanticWithInputDirectRowSerialDotOps ?? "n/a";
    const semanticDirectTotalRowSerialDotOps = data?.lanes?.semantic?.semanticWithInputDirectTotalRowSerialDotOps ?? "n/a";
    const semanticDirectPerRowThreadgroup = data?.lanes?.semantic?.semanticWithInputDirectTotalRowSerialDotOpsPerRowThreadgroup ?? "n/a";
    const semanticDirectWidthParallelCount = q8DirectWidthMetric(data?.lanes?.semantic, "semanticWithInputDirectWidthParallelCount");
    const semanticDirectWidthParallelRows = q8DirectWidthMetric(data?.lanes?.semantic, "semanticWithInputDirectWidthParallelRows");
    const semanticDirectWidthParallelRowTileGroups = q8DirectWidthMetric(data?.lanes?.semantic, "semanticWithInputDirectWidthParallelRowTileGroups");
    const semanticDirectWidthParallelOutputTiles = q8DirectWidthMetric(data?.lanes?.semantic, "semanticWithInputDirectWidthParallelOutputTiles");
    const semanticDirectWidthParallelLanes = q8DirectWidthMetric(data?.lanes?.semantic, "semanticWithInputDirectWidthParallelLanes");
    const semanticDirectWidthParallelPartialSlots = q8DirectWidthMetric(data?.lanes?.semantic, "semanticWithInputDirectWidthParallelPartialSlots");
    const semanticFallbackPairDispatches = data?.lanes?.semantic?.semanticFallbackPairDispatches ?? "n/a";
    const semanticFallbackTailDispatches = data?.lanes?.semantic?.semanticFallbackTailDispatches ?? "n/a";
    const semanticFallbackDispatches = data?.lanes?.semantic?.semanticFallbackDispatches ?? "n/a";
    const semanticFallbackSplit = formatNumber(data?.lanes?.semantic?.semanticFallbackDispatchSplit, 2);
    const semanticInputDispatches = Number(data?.lanes?.semantic?.semanticFfnSublayerWithInputRowChainDispatches) - Number(data?.lanes?.semantic?.semanticFallbackDispatches);
    const semanticInputSplit = Number(data?.lanes?.semantic?.semanticFfnSublayersWithInputRowChain) > 0
      ? semanticInputDispatches / Number(data.lanes.semantic.semanticFfnSublayersWithInputRowChain)
      : NaN;
    ...
    const semanticSingleDispatchRefusals = formatSemanticSingleDispatchRefusals(data?.lanes?.semantic?.singleDispatchRefusals);
...
function q8DirectWidthMetric(lane, key) {
  const value = lane?.[key];
  if (value !== undefined && value !== null) return value;
  return Number(lane?.semanticWithInputDirectCount) === 0 ? 0 : "n/a";
}
...
function qsemanticBridgeArtifacts() {
  ...
  if (!existsSync(dir)) return [];
  return readdirSync(dir)
    .filter((name) => qsemanticBridgeArtifactPattern.test(name))
    .sort()
    .map((name) => join(dir, name))
    .filter((path) => {
      try {
        const data = readJson(path);
        return data?.schema === "zgml.frontier-qsemantic-bridge.v1" &&
          data?.kind === "qsemantic-bridge" &&
          data?.bridgeFfn;
      } catch {
        return false;
      }
...
function qsemanticInputBridgeArtifacts() {
  ...
  if (!existsSync(dir)) return [];
  return readdirSync(dir)
    .filter((name) => qsemanticInputBridgeArtifactPattern.test(name))
    .sort()
    .map((name) => join(dir, name))
    .filter((path) => {
      try {
        const data = readJson(path);
        return data?.schema === "zgml.frontier-qsemantic-input-bridge.v1" &&
          data?.kind === "qsemantic-input-bridge" &&
          data?.absorbed;
      } catch {
        return false;
      }
...
function latestQsemanticInputBridgeArtifact() {
  ...
  const passing = artifacts.filter(isPassingFrontierArtifact);
  const passingWithScratchAllocation = passing.filter(hasQsemanticInputBridgeScratchAllocation);
  return passingWithScratchAllocation.filter(isFrontierSteadyArtifact).at(-1) ??
    passingWithScratchAllocation.at(-1) ??
    passing.filter(isFrontierSteadyArtifact).at(-1) ??
    passing.at(-1) ??
    artifacts.filter(isFrontierSteadyArtifact).at(-1) ??
    artifacts.at(-1) ??
...
function hasQsemanticInputBridgeScratchAllocation(path) {
  try {
    const absorbed = readJson(path)?.absorbed;
    return Number(absorbed?.semanticWidthScratchAllocatedBytes) > 0 &&
      absorbed.semanticWidthScratchAllocatedBytes === absorbed.semanticWidthScratchDownPartialBytes;
  } catch {
    return false;
  }
}
...
function q8PromptSemanticInputSplitFields(path) {
  ...
  try {
    const data = readJson(path);
    const semantic = data?.lanes?.semantic;
    const absorbed = Number(semantic?.semanticFfnSublayersWithInputRowChain);
    const absorbedDispatches = Number(semantic?.semanticFfnSublayerWithInputRowChainDispatches);
    const fallbackDispatches = Number(semantic?.semanticFallbackDispatches);
    if (!Number.isFinite(absorbed) || absorbed <= 0 || !Number.isFinite(absorbedDispatches) || !Number.isFinite(fallbackDispatches)) {
      return null;
    }
    const inputDispatches = absorbedDispatches - fallbackDispatches;
    return {
      absorbed,
      absorbedDispatches,
      inputDispatches,
      inputSplit: inputDispatches / absorbed,
      fallbackDispatches,
      fallbackSplit: fallbackDispatches / absorbed,
    };
  } catch {
    return null;
  }
...
function qsemanticBridgeStatusLine(path, q8PressurePath = null) {
  ...
  try {
    ...
    return `qsemantic-bridge-results: latest=${compactName(path)} unreadable`;
  }
  const status = typeof data?.status === "string" ? data.status : "unknown";
  const selectedAttempt = Number.isInteger(data?.selectedAttempt) ? data.selectedAttempt : "n/a";
  const attempts = Number.isInteger(data?.attempts) ? data.attempts : "n/a";
...
function qsemanticBridgeFreshnessStatusLine(selectedPath, rawPath) {
  ...
  const scratch = `scratch=candidates:${bridge.semanticWidthScratchCandidates ?? "n/a"},bytes:${bridge.semanticWidthScratchBytes ?? "n/a"},allocated:${bridge.semanticWidthScratchAllocatedBytes ?? "n/a"},down_partial_bytes:${bridge.semanticWidthScratchDownPartialBytes ?? "n/a"},down_partial_to_output:${formatNumber(bridge.semanticWidthScratchDownPartialToOutput, 2)},runtime_capacity:${bridge.semanticWidthScratchRuntimeCapacityBytes ?? "n/a"},runtime_uses:${bridge.semanticWidthScratchRuntimeUses ?? "n/a"},runtime_bytes:${bridge.semanticWidthScratchRuntimeBytes ?? "n/a"}`;
...
function qsemanticInputBridgeGate(data, absorbed) {
  if (typeof data?.absorbedGate === "string") return data.absorbedGate;
  const floor = Number(data?.absorbedFloor ?? data?.config?.semanticInputBridgeSteadyAbsorbedSpeedupFloor ?? 2.45);
  const diffCeil = Number(data?.config?.projectionRowChainMaxAbsDiffCeil ?? 0.02);
  const speedup = Number(absorbed?.speedup);
  const diff = Number(absorbed?.maxAbsDiff);
  if (Number.isFinite(speedup) && Number.isFinite(diff) && speedup >= floor && diff <= diffCeil) return "ready";
  if (Number.isFinite(speedup) && Number.isFinite(diff) && speedup >= 1.0 && diff <= diffCeil) return "diagnostic";
  return "below_floor";
}

function qsemanticInputBridgeFloor(data) {
  return formatRatio(Number(data?.absorbedFloor ?? data?.config?.semanticInputBridgeSteadyAbsorbedSpeedupFloor ?? 2.45));
}

function qsemanticInputBridgeAbsorbedProfile(data, absorbed) {
  if (typeof data?.absorbedProfile === "string") return data.absorbedProfile;
  if (typeof absorbed?.profile === "string") return absorbed.profile;
  const dispatches = Number(absorbed?.runtimeDispatches);
  const withInputDispatches = Number(absorbed?.runtimeSemanticFfnWithInputDispatches);
  const direct = Number(absorbed?.semanticWithInputDirectCount);
  const directWidth = Number(absorbed?.semanticWithInputDirectWidthParallelCount);
  const decomposedExtra = Number(absorbed?.semanticWithInputDecomposedExtraDispatches);
  if (dispatches === 1 && withInputDispatches === 1 && directWidth === 1) return "direct_width_promotable";
  if (dispatches === 1 && withInputDispatches === 1 && direct === 1) return "direct_serial_absorbed";
  if (dispatches === 3 && withInputDispatches === 3 && decomposedExtra === 2) return "staged_width_absorbed";
  if (dispatches === 5 && withInputDispatches === 5 && decomposedExtra === 4) return "decomposed_absorbed";
  return "unknown";
}

function qsemanticInputBridgeDirectWidthVsAbsorbed(data, absorbed, directWidth) {
  const recorded = Number(data?.directWidthVsAbsorbed ?? directWidth?.vsAbsorbed);
  if (Number.isFinite(recorded)) return recorded;
  const directWidthSpeedup = Number(directWidth?.speedup);
  const absorbedSpeedup = Number(absorbed?.speedup);
  if (!Number.isFinite(directWidthSpeedup) || !Number.isFinite(absorbedSpeedup) || absorbedSpeedup === 0) return NaN;
  return directWidthSpeedup / absorbedSpeedup;
}

function qsemanticInputBridgeDirectWidthGate(data, absorbed, directWidth) {
  if (typeof data?.directWidthGate === "string") return data.directWidthGate;
  if (typeof directWidth?.gate === "string") return directWidth.gate;
  const speedup = Number(directWidth?.speedup);
  const absorbedSpeedup = Number(absorbed?.speedup);
  const maxAbsDiff = Number(directWidth?.maxAbsDiff);
  const diffCeil = Number(data?.config?.projectionRowChainMaxAbsDiffCeil ?? 0.02);
  if (!Number.isFinite(speedup) || !Number.isFinite(absorbedSpeedup)) return "missing";
  if (!Number.isFinite(maxAbsDiff) || maxAbsDiff > diffCeil) return "diff_fail";
  if (speedup >= absorbedSpeedup) return "ready";
  if (speedup >= 1.0 && Number(directWidth?.semanticWithInputDirectWidthParallelCount) > 0) return "diagnostic";
  return "below_absorbed";
}

function qsemanticInputBridgeScratchSegment(label, record, partialLabel = "down_partial") {
  return `${label}=candidates:${record.semanticWidthScratchCandidates ?? "n/a"},bytes:${record.semanticWidthScratchBytes ?? "n/a"},allocated:${record.semanticWidthScratchAllocatedBytes ?? "n/a"},input_bytes:${record.semanticWidthScratchInputBytes ?? "n/a"},product_bytes:${record.semanticWidthScratchProductBytes ?? "n/a"},${partialLabel}_bytes:${record.semanticWidthScratchDownPartialBytes ?? "n/a"},output_bytes:${record.semanticWidthScratchOutputBytes ?? "n/a"},${partialLabel}_to_output:${formatNumber(record.semanticWidthScratchDownPartialToOutput, 3)},runtime_capacity:${record.semanticWidthScratchRuntimeCapacityBytes ?? "n/a"},runtime_uses:${record.semanticWidthScratchRuntimeUses ?? "n/a"},runtime_bytes:${record.semanticWidthScratchRuntimeBytes ?? "n/a"}`;
}

function qsemanticInputBridgeStatusLine(path) {
  if (!path) {
    return "qsemantic-input-bridge-results: no local absorbed input-row-chain qsemantic artifact found; run npm run dev:perf:frontier:qsemantic:input-bridge:run";
  }
  let data;
  try {
    data = readJson(path);
  } catch {
    return `qsemantic-input-bridge-results: latest=${compactName(path)} unreadable`;
  }
  const status = typeof data?.status === "string" ? data.status : "unknown";
  const selectedAttempt = Number.isInteger(data?.selectedAttempt) ? data.selectedAttempt : "n/a";
  const attempts = Number.isInteger(data?.attempts) ? data.attempts : "n/a";
  ...
  const directWidth = data?.directWidth ?? {};
  const directTarget = data?.directTarget ?? {};
  ...
  const directWidthSpeedup = formatRatio(directWidth.speedup);
  const directWidthMedian = formatRatio(data?.speedupStats?.directWidth?.median);
  const maxAbsDiff = formatNumber(absorbed.maxAbsDiff, 6);
  const gate = qsemanticInputBridgeGate(data, absorbed);
  const floor = qsemanticInputBridgeFloor(data);
  const absorbedProfile = qsemanticInputBridgeAbsorbedProfile(data, absorbed);
  const directWidthGate = qsemanticInputBridgeDirectWidthGate(data, absorbed, directWidth);
  const directWidthVsAbsorbed = formatNumber(qsemanticInputBridgeDirectWidthVsAbsorbed(data, absorbed, directWidth), 2);
  const source = typeof data?.source?.label === "string" ? data.source.label : "unknown";
  const next = typeof data?.next === "string" ? data.next : "unknown";
  const absorbedDirectWidth = Number(absorbed.semanticWithInputDirectWidthParallelCount) > 0;
  const directPartition = absorbedDirectWidth
    ? "width_parallel"
    : Number(directSerial.semanticWithInputDirectCount) > 0
      ? "row_serial"
      : Number(absorbed.semanticWithInputDirectCount) > 0
        ? "absorbed_row_serial"
        : "inactive";
  const scratch = qsemanticInputBridgeScratchSegment("scratch", absorbed);
  const directWidthScratch = qsemanticInputBridgeScratchSegment("direct_width_scratch", directWidth, "partial");
  const directSerialWork = `direct_serial_work=input_projection:${directSerial.semanticWithInputDirectInputProjection ?? "n/a"},input:${directSerial.semanticWithInputDirectInput ?? "n/a"},hidden:${directSerial.semanticWithInputDirectHidden ?? "n/a"},output:${directSerial.semanticWithInputDirectOutput ?? "n/a"},input_projection_dot_ops:${directSerial.semanticWithInputDirectInputProjectionDotOps ?? "n/a"},gate_up_dot_ops:${directSerial.semanticWithInputDirectGateUpDotOps ?? "n/a"},down_dot_ops:${directSerial.semanticWithInputDirectDownDotOps ?? "n/a"}`;
  const target = `direct_target=kernel:${directTarget.kernel ?? "semantic_with_input_width_parallel_kernel"},rows:${directTarget.rows ?? "n/a"},input:${directTarget.input ?? "n/a"},hidden:${directTarget.hidden ?? "n/a"},output:${directTarget.output ?? "n/a"},row_groups:${directTarget.rowGroups ?? "n/a"},hidden_tiles:${directTarget.hiddenTiles ?? "n/a"},output_tiles:${directTarget.outputTiles ?? "n/a"},width_lanes:${directTarget.widthLanes ?? "n/a"},partial_slots:${directTarget.directWidthPartialSlots ?? "n/a"},input_projection_dot_ops:${directTarget.inputProjectionDotOps ?? "n/a"},gate_up_dot_ops:${directTarget.gateUpDotOps ?? "n/a"},down_dot_ops:${directTarget.downDotOps ?? "n/a"},total_dot_ops:${directTarget.totalDotOps ?? "n/a"},total_rows_dot_ops:${directTarget.totalRowsDotOps ?? "n/a"}`;
  return `qsemantic-input-bridge-results: latest=${compactName(path)} status=${status} gate=${gate} absorbed_profile=${absorbedProfile} direct_width_gate=${directWidthGate} direct_width_vs_absorbed=${directWidthVsAbsorbed} floor=${floor} attempt=${selectedAttempt}/${attempts} command=${commandSpeedup}:dispatches:${command.runtimeDispatches ?? "n/a"}:bridges:${command.shapeProjectionRowChainSemanticResidualBridges ?? "n/a"} absorbed=${absorbedSpeedup}:median:${absorbedMedian}:worst:${absorbedWorst}:max_abs_diff:${maxAbsDiff}:dispatches:${absorbed.runtimeDispatches ?? "n/a"}:semantic_with_input_dispatches:${absorbed.runtimeSemanticFfnWithInputDispatches ?? "n/a"}:absorbed_split:${formatNumber(absorbed.absorbedDispatchSplit, 2)}:decomposed:${absorbed.semanticWithInputDecomposedCount ?? "n/a"}:decomposed_dispatches:${absorbed.semanticWithInputDecomposedDispatches ?? "n/a"}:decomposed_extra:${absorbed.semanticWithInputDecomposedExtraDispatches ?? "n/a"}:decomposed_row_chain:${absorbed.semanticWithInputDecomposedRowChainDispatches ?? "n/a"}:decomposed_pair:${absorbed.semanticWithInputDecomposedPairDispatches ?? "n/a"}:decomposed_tail:${absorbed.semanticWithInputDecomposedTailDispatches ?? "n/a"}:direct_partition:${directPartition}:direct:${absorbed.semanticWithInputDirectCount ?? "n/a"}:direct_width_parallel:${absorbed.semanticWithInputDirectWidthParallelCount ?? "n/a"}:direct_width_lanes:${absorbed.semanticWithInputDirectWidthParallelLanes ?? "n/a"}:direct_width_rows:${absorbed.semanticWithInputDirectWidthParallelRows ?? "n/a"}:direct_width_tiles:${absorbed.semanticWithInputDirectWidthParallelRowTileGroups ?? "n/a"}x${absorbed.semanticWithInputDirectWidthParallelOutputTiles ?? "n/a"}:direct_width_partial_slots:${absorbed.semanticWithInputDirectWidthParallelPartialSlots ?? "n/a"} direct_serial=${directSerialSpeedup}:median:${directSerialMedian}:dispatches:${directSerial.runtimeDispatches ?? "n/a"}:direct:${directSerial.semanticWithInputDirectCount ?? "n/a"}:rows:${directSerial.semanticWithInputDirectRows ?? "n/a"}:row_threadgroups:${directSerial.semanticWithInputDirectRowThreadgroups ?? "n/a"}:row_serial_dot_ops:${directSerial.semanticWithInputDirectRowSerialDotOps ?? "n/a"}:total_row_serial_dot_ops:${directSerial.semanticWithInputDirectTotalRowSerialDotOps ?? "n/a"}:per_row_threadgroup:${directSerial.semanticWithInputDirectTotalRowSerialDotOpsPerRowThreadgroup ?? "n/a"}:fallback_pair:${directSerial.semanticFallbackPairDispatches ?? "n/a"}:fallback_tail:${directSerial.semanticFallbackTailDispatches ?? "n/a"}:tiled:${directSerial.qmatmulRowChainTiledCount ?? "n/a"}:width_parallel:${directSerial.qmatmulRowChainWidthParallelCount ?? "n/a"} direct_width=${directWidthSpeedup}:median:${directWidthMedian}:dispatches:${directWidth.runtimeDispatches ?? "n/a"}:direct:${directWidth.semanticWithInputDirectCount ?? "n/a"}:width_parallel:${directWidth.semanticWithInputDirectWidthParallelCount ?? "n/a"}:width_lanes:${directWidth.semanticWithInputDirectWidthParallelLanes ?? "n/a"}:width_rows:${directWidth.semanticWithInputDirectWidthParallelRows ?? "n/a"}:width_tiles:${directWidth.semanticWithInputDirectWidthParallelRowTileGroups ?? "n/a"}x${directWidth.semanticWithInputDirectWidthParallelOutputTiles ?? "n/a"}:partial_slots:${directWidth.semanticWithInputDirectWidthParallelPartialSlots ?? "n/a"} ${directWidthScratch} ${directSerialWork} ${target} pair_dispatches=${absorbed.semanticFallbackPairDispatches ?? "n/a"}:tail_dispatches:${absorbed.semanticFallbackTailDispatches ?? "n/a"}:tiled:${absorbed.qmatmulRowChainTiledCount ?? "n/a"}:row_tile_groups:${absorbed.qmatmulRowChainTiledRowTileGroups ?? "n/a"}:n_tiles:${absorbed.qmatmulRowChainTiledNTiles ?? "n/a"}:two_phase:${absorbed.qmatmulRowChainTiledTwoPhaseCount ?? "n/a"}:width_parallel:${absorbed.qmatmulRowChainWidthParallelCount ?? "n/a"}:width_lanes:${absorbed.qmatmulRowChainWidthParallelLanes ?? "n/a"}:spilled_input:${absorbed.qmatmulRowChainTiledSpilledInput ?? "n/a"}:output_spills:${absorbed.qmatmulRowChainTiledOutputSpills ?? "n/a"} ${scratch} next=${next} source=${source}`;
}
...
function qsemanticInputBridgeFreshnessStatusLine(selectedPath, rawPath) {
  ...
  const directWidthScratch = qsemanticInputBridgeScratchSegment("direct_width_scratch", directWidth, "partial");
  const directSerialWork = `direct_serial_work=input_projection:${directSerial.semanticWithInputDirectInputProjection ?? "n/a"},input:${directSerial.semanticWithInputDirectInput ?? "n/a"},hidden:${directSerial.semanticWithInputDirectHidden ?? "n/a"},output:${directSerial.semanticWithInputDirectOutput ?? "n/a"},input_projection_dot_ops:${directSerial.semanticWithInputDirectInputProjectionDotOps ?? "n/a"},gate_up_dot_ops:${directSerial.semanticWithInputDirectGateUpDotOps ?? "n/a"},down_dot_ops:${directSerial.semanticWithInputDirectDownDotOps ?? "n/a"}`;
  const target = `direct_target=rows:${directTarget.rows ?? "n/a"},input:${directTarget.input ?? "n/a"},hidden:${directTarget.hidden ?? "n/a"},output:${directTarget.output ?? "n/a"},width_lanes:${directTarget.widthLanes ?? "n/a"},partial_slots:${directTarget.directWidthPartialSlots ?? "n/a"},total_dot_ops:${directTarget.totalDotOps ?? "n/a"}`;
  return `qsemantic-input-bridge-latest-results: newest=${compactName(rawPath)} selected=${compactName(selectedPath)} reason=prefer_passing_steady_attempts gate=${gate} absorbed_profile=${absorbedProfile} direct_width_gate=${directWidthGate} direct_width_vs_absorbed=${directWidthVsAbsorbed} floor=${floor} attempt=${selectedAttempt}/${attempts} command=${commandSpeedup}:dispatches:${command.runtimeDispatches ?? "n/a"} absorbed=${absorbedSpeedup}:median:${absorbedMedian}:worst:${absorbedWorst}:max_abs_diff:${maxAbsDiff}:dispatches:${absorbed.runtimeDispatches ?? "n/a"}:semantic_with_input_dispatches:${absorbed.runtimeSemanticFfnWithInputDispatches ?? "n/a"}:absorbed_split:${formatNumber(absorbed.absorbedDispatchSplit, 2)}:decomposed:${absorbed.semanticWithInputDecomposedCount ?? "n/a"}:decomposed_dispatches:${absorbed.semanticWithInputDecomposedDispatches ?? "n/a"}:decomposed_extra:${absorbed.semanticWithInputDecomposedExtraDispatches ?? "n/a"}:direct_width_parallel:${absorbed.semanticWithInputDirectWidthParallelCount ?? "n/a"}:direct_width_lanes:${absorbed.semanticWithInputDirectWidthParallelLanes ?? "n/a"}:direct_width_rows:${absorbed.semanticWithInputDirectWidthParallelRows ?? "n/a"}:direct_width_tiles:${absorbed.semanticWithInputDirectWidthParallelRowTileGroups ?? "n/a"}x${absorbed.semanticWithInputDirectWidthParallelOutputTiles ?? "n/a"}:direct_width_partial_slots:${absorbed.semanticWithInputDirectWidthParallelPartialSlots ?? "n/a"} direct_serial=${directSerialSpeedup}:median:${directSerialMedian}:dispatches:${directSerial.runtimeDispatches ?? "n/a"}:direct:${directSerial.semanticWithInputDirectCount ?? "n/a"}:row_serial_dot_ops:${directSerial.semanticWithInputDirectRowSerialDotOps ?? "n/a"}:total_row_serial_dot_ops:${directSerial.semanticWithInputDirectTotalRowSerialDotOps ?? "n/a"} direct_width=${directWidthSpeedup}:median:${directWidthMedian}:dispatches:${directWidth.runtimeDispatches ?? "n/a"}:direct:${directWidth.semanticWithInputDirectCount ?? "n/a"}:width_parallel:${directWidth.semanticWithInputDirectWidthParallelCount ?? "n/a"}:width_lanes:${directWidth.semanticWithInputDirectWidthParallelLanes ?? "n/a"}:width_tiles:${directWidth.semanticWithInputDirectWidthParallelRowTileGroups ?? "n/a"}x${directWidth.semanticWithInputDirectWidthParallelOutputTiles ?? "n/a"}:partial_slots:${directWidth.semanticWithInputDirectWidthParallelPartialSlots ?? "n/a"} ${directWidthScratch} ${directSerialWork} ${target} pair_dispatches=${absorbed.semanticFallbackPairDispatches ?? "n/a"}:tail_dispatches:${absorbed.semanticFallbackTailDispatches ?? "n/a"}:tiled:${absorbed.qmatmulRowChainTiledCount ?? "n/a"}:width_parallel:${absorbed.qmatmulRowChainWidthParallelCount ?? "n/a"}:width_lanes:${absorbed.qmatmulRowChainWidthParallelLanes ?? "n/a"} ${scratch} next=${next} source=${source}`;
}
...
function qsemanticInputBridgeNextTargetLine(path) {
  ...
  try {
    ...
    const directWidthGate = qsemanticInputBridgeDirectWidthGate(data, absorbed, directWidth);
    const directWidthVsAbsorbed = formatNumber(qsemanticInputBridgeDirectWidthVsAbsorbed(data, absorbed, directWidth), 2);
    return `qsemantic_input_bridge=absorbed:${formatRatio(absorbed.speedup)}:median:${formatRatio(data?.speedupStats?.absorbed?.median)}:gate:${gate}:floor:${floor}:absorbed_profile:${absorbedProfile}:direct_width_gate:${directWidthGate}:direct_width_vs_absorbed:${directWidthVsAbsorbed}:dispatches:${absorbed.runtimeDispatches ?? "n/a"}:decomposed_extra:${absorbed.semanticWithInputDecomposedExtraDispatches ?? "n/a"}:direct_width_parallel:${absorbed.semanticWithInputDirectWidthParallelCount ?? "n/a"}:direct_width_lanes:${absorbed.semanticWithInputDirectWidthParallelLanes ?? "n/a"}:direct_width_rows:${absorbed.semanticWithInputDirectWidthParallelRows ?? "n/a"}:direct_width_tiles:${absorbed.semanticWithInputDirectWidthParallelRowTileGroups ?? "n/a"}x${absorbed.semanticWithInputDirectWidthParallelOutputTiles ?? "n/a"}:direct_width_partial_slots:${absorbed.semanticWithInputDirectWidthParallelPartialSlots ?? "n/a"}:target_shape:${directTarget.rows ?? "n/a"}x${directTarget.input ?? "n/a"}x${directTarget.hidden ?? "n/a"}x${directTarget.output ?? "n/a"}:target_row_groups:${directTarget.rowGroups ?? "n/a"}:target_hidden_tiles:${directTarget.hiddenTiles ?? "n/a"}:target_output_tiles:${directTarget.outputTiles ?? "n/a"}:target_width_lanes:${directTarget.widthLanes ?? "n/a"}:target_partial_slots:${directTarget.directWidthPartialSlots ?? "n/a"}:target_down_partial_to_output:${formatNumber(directTarget.downPartialToOutput, 2)}:target_input_bytes:${directTarget.inputBytes ?? "n/a"}:target_direct_width_partial_bytes:${directTarget.directWidthPartialBytes ?? "n/a"}:target_direct_width_scratch_bytes:${directTarget.directWidthScratchBytes ?? "n/a"}:target_total_dot_ops:${directTarget.totalDotOps ?? "n/a"}:scratch_bytes:${absorbed.semanticWidthScratchBytes ?? "n/a"}:scratch_allocated:${absorbed.semanticWidthScratchAllocatedBytes ?? "n/a"}:scratch_input_bytes:${absorbed.semanticWidthScratchInputBytes ?? "n/a"}:scratch_down_partial_bytes:${absorbed.semanticWidthScratchDownPartialBytes ?? "n/a"}:scratch_down_partial_to_output:${formatNumber(absorbed.semanticWidthScratchDownPartialToOutput, 2)}:scratch_runtime_uses:${absorbed.semanticWidthScratchRuntimeUses ?? "n/a"}:scratch_runtime_bytes:${absorbed.semanticWidthScratchRuntimeBytes ?? "n/a"}:direct_serial:${formatRatio(directSerial.speedup)}:direct_serial_median:${formatRatio(data?.speedupStats?.directSerial?.median)}:direct_serial_dispatches:${directSerial.runtimeDispatches ?? "n/a"}:row_serial_dot_ops:${directSerial.semanticWithInputDirectRowSerialDotOps ?? "n/a"}:input_projection_dot_ops:${directSerial.semanticWithInputDirectInputProjectionDotOps ?? "n/a"}:gate_up_dot_ops:${directSerial.semanticWithInputDirectGateUpDotOps ?? "n/a"}:down_dot_ops:${directSerial.semanticWithInputDirectDownDotOps ?? "n/a"}:total_row_serial_dot_ops:${directSerial.semanticWithInputDirectTotalRowSerialDotOps ?? "n/a"}:direct_width:${formatRatio(directWidth.speedup)}:direct_width_median:${formatRatio(data?.speedupStats?.directWidth?.median)}:direct_width_dispatches:${directWidth.runtimeDispatches ?? "n/a"}:direct_width_kernel_count:${directWidth.semanticWithInputDirectWidthParallelCount ?? "n/a"}:direct_width_kernel_lanes:${directWidth.semanticWithInputDirectWidthParallelLanes ?? "n/a"}:direct_width_kernel_tiles:${directWidth.semanticWithInputDirectWidthParallelRowTileGroups ?? "n/a"}x${directWidth.semanticWithInputDirectWidthParallelOutputTiles ?? "n/a"}:direct_width_kernel_partial_slots:${directWidth.semanticWithInputDirectWidthParallelPartialSlots ?? "n/a"}:direct_width_scratch_bytes:${directWidth.semanticWidthScratchBytes ?? "n/a"}:direct_width_scratch_allocated:${directWidth.semanticWidthScratchAllocatedBytes ?? "n/a"}:direct_width_scratch_input_bytes:${directWidth.semanticWidthScratchInputBytes ?? "n/a"}:direct_width_scratch_partial_bytes:${directWidth.semanticWidthScratchDownPartialBytes ?? "n/a"}:direct_width_scratch_runtime_capacity:${directWidth.semanticWidthScratchRuntimeCapacityBytes ?? "n/a"}:direct_width_scratch_runtime_uses:${directWidth.semanticWidthScratchRuntimeUses ?? "n/a"}:direct_width_scratch_runtime_bytes:${directWidth.semanticWidthScratchRuntimeBytes ?? "n/a"}:next=${next}:source=${source}`;
  } catch {
...
function q8PromptNextTarget(path, pressurePath = path, bridgePath = null) {
  ...
  try {
    ...
    const freshSemanticDirectCount = pressureData?.lanes?.semantic?.semanticWithInputDirectCount ?? "n/a";
    const freshSemanticDirectRows = pressureData?.lanes?.semantic?.semanticWithInputDirectRows ?? "n/a";
    const freshSemanticDirectRowThreadgroups = pressureData?.lanes?.semantic?.semanticWithInputDirectRowThreadgroups ?? "n/a";
    const freshSemanticDirectRowSerialDotOps = pressureData?.lanes?.semantic?.semanticWithInputDirectRowSerialDotOps ?? "n/a";
    const freshSemanticDirectTotalRowSerialDotOps = pressureData?.lanes?.semantic?.semanticWithInputDirectTotalRowSerialDotOps ?? "n/a";
    const freshSemanticDirectPerRowThreadgroup = pressureData?.lanes?.semantic?.semanticWithInputDirectTotalRowSerialDotOpsPerRowThreadgroup ?? "n/a";
    const freshSemanticDirectWidthParallelCount = q8DirectWidthMetric(pressureData?.lanes?.semantic, "semanticWithInputDirectWidthParallelCount");
    const freshSemanticDirectWidthParallelRows = q8DirectWidthMetric(pressureData?.lanes?.semantic, "semanticWithInputDirectWidthParallelRows");
    const freshSemanticDirectWidthParallelRowTileGroups = q8DirectWidthMetric(pressureData?.lanes?.semantic, "semanticWithInputDirectWidthParallelRowTileGroups");
    const freshSemanticDirectWidthParallelOutputTiles = q8DirectWidthMetric(pressureData?.lanes?.semantic, "semanticWithInputDirectWidthParallelOutputTiles");
    const freshSemanticDirectWidthParallelLanes = q8DirectWidthMetric(pressureData?.lanes?.semantic, "semanticWithInputDirectWidthParallelLanes");
    const freshSemanticDirectWidthParallelPartialSlots = q8DirectWidthMetric(pressureData?.lanes?.semantic, "semanticWithInputDirectWidthParallelPartialSlots");
    const freshSemanticFallbackPairDispatches = pressureData?.lanes?.semantic?.semanticFallbackPairDispatches ?? "n/a";
    const freshSemanticFallbackTailDispatches = pressureData?.lanes?.semantic?.semanticFallbackTailDispatches ?? "n/a";
    const freshSemanticFallbackDispatches = pressureData?.lanes?.semantic?.semanticFallbackDispatches ?? "n/a";
    const freshSemanticFallbackSplit = formatNumber(pressureData?.lanes?.semantic?.semanticFallbackDispatchSplit, 2);
    const freshSemanticInputDispatches = Number(pressureData?.lanes?.semantic?.semanticFfnSublayerWithInputRowChainDispatches) - Number(pressureData?.lanes?.semantic?.semanticFallbackDispatches);
    const freshSemanticInputSplit = Number(pressureData?.lanes?.semantic?.semanticFfnSublayersWithInputRowChain) > 0
      ? freshSemanticInputDispatches / Number(pressureData.lanes.semantic.semanticFfnSublayersWithInputRowChain)
      : NaN;
    ...
    const freshDimRefusalShape = formatSemanticSingleDispatchDimRefusalShape(pressureData?.lanes?.semantic?.singleDispatchDimRefusalShape);
    ...
    const pressureSemanticBestSpeedup = Number(pressureData?.lanes?.semantic?.speedup);
    ...
    const directWork = `:direct=${freshSemanticDirectCount}:direct_rows=${freshSemanticDirectRows}:direct_row_threadgroups=${freshSemanticDirectRowThreadgroups}:direct_row_serial_dot_ops=${freshSemanticDirectRowSerialDotOps}:direct_total_row_serial_dot_ops=${freshSemanticDirectTotalRowSerialDotOps}:direct_per_row_threadgroup=${freshSemanticDirectPerRowThreadgroup}:direct_width_parallel=${freshSemanticDirectWidthParallelCount}:direct_width_rows=${freshSemanticDirectWidthParallelRows}:direct_width_lanes=${freshSemanticDirectWidthParallelLanes}:direct_width_tiles=${freshSemanticDirectWidthParallelRowTileGroups}x${freshSemanticDirectWidthParallelOutputTiles}:direct_width_partial_slots=${freshSemanticDirectWidthParallelPartialSlots}`;
