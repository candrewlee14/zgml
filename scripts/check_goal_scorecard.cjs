...
function checkDocs() {
  ...
  requireIncludes(plan, "docs/executable-stencil-runtime-plan.md", "current goal progress accounting", [
    ...
    "fused two-row small-direct classifier kernel",
    "stable row max/exp sum",
    "C ABI coverage for the `M=128,N=32,K=64` comparison shape",
    "ratio_median=linear_batched:1.23x,log_softmax_classifier_batched:0.94x",
    "log_softmax_classifier_batched=zgml:0.0086ms",
    "pytorch:0.0083ms",
    "latest fresh-native three-attempt PyTorch gap microscope",
    "ratio_median=linear_batched:1.25x",
    "`prepared_execute_into_ms=0.0079ms`",
    "fused two-row small-direct `Linear -> LogSoftmax` classifier-tail",
