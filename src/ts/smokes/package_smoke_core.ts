...
function expectNativeEagerLinearEvidence(adapter: Record<string, any>, label: string) {
  ...
  if (gatherResult !== gatherOutput) {
    throw new Error(`${label} expected nativeEager.gatherInto to reuse caller output`);
  }
  expectClose(gatherOutput, [3, 2, 4, 4], `${label} nativeEager.gatherInto output`);
  const gatherAliasOutput = new Float32Array(4);
  const gatherAliasResult = nativeEagerAlias.gather_into(gatherAliasOutput, adapter.tensor([1, 2, 3, 4], [2, 2]), new Uint32Array([1, 1, 0, 0]), {
    output_shape: new Uint32Array([2, 2]),
    input_strides: new Uint32Array([2, 1]),
    axis: 0,
    axis_len: 2,
  });
  if (gatherAliasResult !== gatherAliasOutput) {
  ...
  const tensorUnaryElementwise = adapter.noGrad(() => tensorElementwiseInput.neg().abs().add(0.25).minimum(1.5).reciprocal());
  expectClose(
    tensorUnaryElementwise.data,
    tensorElementwiseInputData.map((value) => 1 / Math.min(Math.abs(-value) + 0.25, 1.5)),
    `${label} noGrad Tensor unary/minimum/reciprocal native eager output`,
  );
  ...
  const tensorReduceDimInput = adapter.tensor([1, 2, 3, 4, 5, 6], [2, 3]);
  const tensorReduceDim = adapter.noGrad(() => tensorReduceDimInput.sumDim(1).add(tensorReduceDimInput.meanDim(1)).add(tensorReduceDimInput.maxDim(1)).add(tensorReduceDimInput.minDim(1)));
  expectClose(tensorReduceDim.data, [12, 30], `${label} noGrad Tensor dim-reduce native eager output`);
  const reduceDimOutput = new Float32Array(2);
  const reduceDimResult = nativeEager.reduceDimInto(reduceDimOutput, tensorReduceDimInput, { op: "sum", outer: 2, reduce: 3, inner: 1 });
  if (reduceDimResult !== reduceDimOutput) {
    throw new Error(`${label} expected nativeEager.reduceDimInto to reuse caller output`);
  }
  expectClose(reduceDimOutput, [6, 15], `${label} nativeEager.reduceDimInto output`);
  const reduceDimAliasOutput = new Float32Array(2);
  const reduceDimAliasResult = nativeEagerAlias.reduce_dim_into(reduceDimAliasOutput, tensorReduceDimInput, { op: "max", outer: 2, reduce: 3, inner: 1 });
  if (reduceDimAliasResult !== reduceDimAliasOutput) {
...
function expectTorchNamespaceEndToEndEvidence(adapter: Record<string, any>, label: string) {
  ...
  if (
    ...
    lazyNamespaceNaturalAffineGraph.compileSupport().supported !== true ||
    ...
    lazyAddReluGraph.kernelPlan()?.opCount !== 2 ||
    lazyAddReluGraph.kernelPlan()?.dispatchCount !== 1 ||
    lazyAddReluGraph.kernelPlan()?.ops.length !== 1 ||
    lazyAddReluGraph.kernelPlan()?.ops[0]?.fusedOpCount !== 2 ||
    ...
    lazyMulReluGraph.kernelPlan()?.opCount !== 2 ||
    lazyMulReluGraph.kernelPlan()?.dispatchCount !== 1 ||
    lazyMulReluGraph.kernelPlan()?.ops.length !== 1 ||
    lazyMulReluGraph.kernelPlan()?.ops[0]?.fusedOpCount !== 2 ||
    ...
    lazyNaturalAffineGraph.tensorProgramIr()?.ops.map((op: Record<string, any>) => op.op).join("|") !== "mul|add" ||
    ...
    lazyMatmulScaleCompiledProgram.compileEvidence()?.kernelPlan?.dispatchCount !== 2 ||
    lazyMatmulScaleCompiledProgram.compileEvidence()?.kernelPlan?.ops.length !== 2 ||
    lazyMatmulScaleCompiledProgram.compileEvidence()?.kernelPlan?.ops[1]?.fusedOpCount !== 2 ||
    ...
    lazyAddReluCompiledProgram.outputShape().join("x") !== "2x3" ||
    lazyAddReluCompiledProgram.compileEvidence()?.kernelPlan?.opCount !== 2 ||
    lazyAddReluCompiledProgram.compileEvidence()?.kernelPlan?.dispatchCount !== 1 ||
    lazyAddReluCompiledProgram.compileEvidence()?.kernelPlan?.ops[0]?.nativeKernels.join("|") !== "add|relu" ||
    lazyMulReluCompiledProgram.outputShape().join("x") !== "2x3" ||
    lazyMulReluCompiledProgram.compileEvidence()?.kernelPlan?.opCount !== 2 ||
    lazyMulReluCompiledProgram.compileEvidence()?.kernelPlan?.dispatchCount !== 1 ||
    lazyMulReluCompiledProgram.compileEvidence()?.kernelPlan?.ops[0]?.nativeKernels.join("|") !== "mul|relu" ||
    ...
    lazyNaturalAffineCompiledProgram.compileEvidence()?.kernelPlan?.dispatchCount !== 1 ||
    lazyNaturalAffineCompiledProgram.compileEvidence()?.kernelPlan?.ops.length !== 1 ||
    lazyNaturalAffineCompiledProgram.compileEvidence()?.kernelPlan?.ops[0]?.fusedOpCount !== 2 ||
  ...
  const lazyAddReluSession = lazyAddReluCompiledProgram.bind({
    bias: new Float32Array([-2, 1, -5]),
  });
  try {
    expectClose(
      lazyAddReluSession.stepTensor(torch.tensor([1, 2, 3, 4, 5, 6], [2, 3])).data,
      [0, 3, 0, 2, 6, 1],
      `${label} lazy add relu compiled output`,
    );
  } finally {
    lazyAddReluSession.dispose();
  }
  const lazyMulReluSession = lazyMulReluCompiledProgram.bind({
    weights: new Float32Array([2, -1, 0.5]),
  });
  try {
    expectClose(
      lazyMulReluSession.stepTensor(torch.tensor([1, 2, 3, 4, 5, 6], [2, 3])).data,
      [2, 0, 1.5, 8, 0, 3],
      `${label} lazy mul relu compiled output`,
    );
  } finally {
    lazyMulReluSession.dispose();
  }
  ...
  try {
    expectClose(
      lazyNaturalAffineSession.stepTensor(torch.tensor([1, 2, 3, 4, 5, 6], [2, 3])).data,
      [3, 8, -0.5, 9, 5, 1],
      `${label} lazy natural mul-add affine compiled output`,
    );
  } finally {
    lazyNaturalAffineSession.dispose();
  }
  ...
  lazyAddReluCompiledProgram.free();
  lazyMulReluCompiledProgram.free();
...
export function smokePackage(adapter: Record<string, any>, label: string) {
  ...
  if (
    ...
    nativeCoreEvidence.boundary?.moduleCompiler !== "zig-module-program" ||
    nativeCoreEvidence.boundary?.compileEvidence !== "native-program-inspection" ||
  ...
  expectClose(
    adapter.run(inferenceModel, inferenceInput, { backend: "cpu", inputShape: [2] }).data,
    [-0.5],
    `${label} root run one-shot native Program/Session output`,
  );
  expectClose(
    adapter.compile.run(inferenceModel, inferenceInput, { backend: "cpu", inputShape: [2] }).data,
    [-0.5],
