...
fn writeU32(io: std.Io, file: std.Io.File, offset: *u64, val: u32) !void {
    const le = std.mem.nativeToLittle(u32, val);
    try file.writePositionalAll(io, std.mem.asBytes(&le), offset.*);
    offset.* += 4;
}

fn readU32(io: std.Io, file: std.Io.File, offset: *u64) !u32 {
...
pub fn save(comptime T: type, params: []const *Tensor(T), path: []const u8, io: std.Io) !void {
    const file = try createFile(io, path);
    defer file.close(io);
    var offset: u64 = 0;

    // Header
    try writeU32(io, file, &offset, MAGIC);
    try writeU32(io, file, &offset, VERSION);
    try writeU32(io, file, &offset, @intCast(params.len));

    // Parameters
    for (params) |param| {
        // Shape
        try writeU32(io, file, &offset, param.n_dims);
        for (param.ne) |dim| {
            try writeU32(io, file, &offset, @intCast(dim));
        }
        // Data (raw bytes)
...
pub fn load(comptime T: type, params: []const *Tensor(T), path: []const u8, io: std.Io) !void {
    const file = try openFile(io, path);
    defer file.close(io);
    var offset: u64 = 0;

    // Header
    const magic = try readU32(io, file, &offset);
    if (magic != MAGIC) return error.InvalidCheckpoint;
    const version = try readU32(io, file, &offset);
    if (version != VERSION) return error.UnsupportedVersion;
    const n_params = try readU32(io, file, &offset);
    if (n_params != params.len) return error.ParamCountMismatch;

    // Parameters
    for (params) |param| {
        // Shape — read and verify
        const n_dims = try readU32(io, file, &offset);
        if (n_dims != param.n_dims) return error.ShapeMismatch;
        for (param.ne) |dim| {
            const saved_dim = try readU32(io, file, &offset);
...
test "checkpoint - save and load roundtrip" {
    ...
    try save(f32, &params, "/tmp/zgml_test_ckpt.bin", test_io);
    ...
    // Load
    try load(f32, &params, "/tmp/zgml_test_ckpt.bin", test_io);
    ...
    deleteFile(test_io, "/tmp/zgml_test_ckpt.bin") catch {};
}
...
test "checkpoint - shape mismatch is detected" {
    ...
    try save(f32, &.{a}, "/tmp/zgml_test_ckpt2.bin", test_io);

    const b = try Tensor(f32).init(alloc, &.{ 2, 3 });
    defer b.deinit();

    const result = load(f32, &.{b}, "/tmp/zgml_test_ckpt2.bin", test_io);
    try testing.expectError(error.ShapeMismatch, result);

    deleteFile(test_io, "/tmp/zgml_test_ckpt2.bin") catch {};
}
...
test "checkpoint - param count mismatch is detected" {
    ...
    try save(f32, &.{a}, "/tmp/zgml_test_ckpt3.bin", test_io);
    ...
    const result = load(f32, &.{ b, c }, "/tmp/zgml_test_ckpt3.bin", test_io);
    try testing.expectError(error.ParamCountMismatch, result);

    deleteFile(test_io, "/tmp/zgml_test_ckpt3.bin") catch {};
}
