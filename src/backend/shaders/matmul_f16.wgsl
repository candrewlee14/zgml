// Register-tiled f32×f16 matmul — A is f32, B is packed f16 (2 values per u32).
// Same 2D block tiling as matmul.wgsl. B weights dequantized via unpack2x16float
// during cooperative load into shared memory. Accumulation is pure f32.
...
@group(0) @binding(1) var<storage, read>       B : array<u32>;  // packed f16 pairs
@group(0) @binding(2) var<storage, read_write> C : array<f32>;
@group(0) @binding(3) var<uniform>             p : MatMulF16Params;
...
fn main(
    ...
    for (var kt: u32 = 0u; kt < p.K; kt += BK) {
        ...
        // -- Cooperative load A tile [BM x BK] into tA (padded stride 17) --
        // A is f32, row-major with a_offset.
        for (var i = 0u; i < LOADS_PER_THREAD; i++) {
            ...
            if (g_row < p.M && g_col < p.K) {
                tA[a_r * BK_PAD + a_c] = A[p.a_offset + g_row * p.K + g_col];
            } else {
                tA[a_r * BK_PAD + a_c] = 0.0;
            }
        }

        // -- Cooperative load B tile [BK x BN] with f16 unpacking --
        // B is packed f16: array<u32>, row-major, contiguous (K x N elements).
        // Each u32 holds 2 consecutive f16 values.
        for (var i = 0u; i < LOADS_PER_THREAD; i++) {
            let flat = tid + i * N_THREADS;
            let b_r = flat / BN;
            let b_c = flat % BN;
            ...
            if (g_row < p.K && g_col < p.N) {
                let elem_idx = g_row * p.N + g_col;
                let packed_idx = elem_idx / 2u;
                let lane = elem_idx % 2u;
                let pair = unpack2x16float(B[packed_idx]);
                tB[b_r * BN + b_c] = select(pair.x, pair.y, lane == 1u);
            } else {
                tB[b_r * BN + b_c] = 0.0;
            }
        }
