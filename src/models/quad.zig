...
fn mseFunc(comptime T: type, x: *Tensor(T), y: *Tensor(T)) Alloc.Error!*Tensor(T) {
    const diff = try x.sub(y);
    const diff2 = try diff.sqr();
    return try diff2.mean(&.{1}); // TODO: switch to mean
}
...
pub fn Model(comptime T: type) type {
    return struct {
        ...
        //     try testing.expectEqual(@as(T, 5), model.params[0].data[0]);
        // }
    };
}
