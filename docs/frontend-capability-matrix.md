...
| Program/Session hot path | n/a | n/a | yes | yes | `zgml.compileInference` is the friendly first-contact handle; `compile.compileForInference` remains the explicit namespace form, and `compile -> Program -> bind -> Session -> step/executeInto` remains available for inspection and deployment control. |
...
- Native eager tensor storage is not the default. Large tensor performance
  ...
  `lazy_matmul_add_silu_batched` production-activation targets, as eager TS tensor
