...
| Scalar unary math | checked | `Tensor.reciprocal autograd backward`, `Tensor.rsqrt autograd backward`, `Tensor.sin autograd backward`, `Tensor.tanh autograd backward` |
...
| Log-sum-exp family | checked | `Tensor.logsumexp autograd backward` |
...
- Add numerical-gradient checks for the most nonlinear TS frontend paths that
  currently have exact-value smoke checks only.
