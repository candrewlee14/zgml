...
`bufferLayout()`, buffer-slot lookup, shape/proof helpers, and allocation-free
`into(...)`. That is the desired split: ergonomic JS/TS API, Zig-owned
execution core, no mirrored Zig product frontend.
...
- zgml frontend replacement feel: ~85%. The TS-owned product frontend now has
  ...
  uses vectorized Zig helpers for unary, scalar, and same-shape
  ...
  as well: the expected row set is `row_coverage=38/38` after adding direct
  ...
  `elementwise_abs_batched`, `elementwise_sqrt_batched`,
  `elementwise_reciprocal_batched`, `elementwise_rsqrt_batched`,
