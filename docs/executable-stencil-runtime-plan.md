...
- PyTorch-like replacement feel: ~88%. The TS-owned product frontend now has
  ...
  benchmarked allocation-free lazy `matmul -> add -> relu`, MLP, reduced MLP,
  classifier log-softmax, classifier softmax-reduction, transformer FFN, and
  token-head Session paths,
