...
| MNIST MLP benchmark smoke | Train a standard MNIST `784 -> 128 -> 10` classifier with cross entropy, assert held-out loss/accuracy, checkpoint restore, and compile a trained single-image inference Program. | `examples/node_training/train_mnist_mlp.cjs`; `smoke:training:mnist`; `bench:mnist:pytorch`; bounded cached IDX download; PyTorch-matched loss/accuracy/logit parity; eager/restored/compiled logits parity. | Training math matches PyTorch, but the JS/TS training loop is not yet PyTorch-speed competitive. |
